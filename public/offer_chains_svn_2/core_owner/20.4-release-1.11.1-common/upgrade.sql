...
CREATE OR REPLACE PACKAGE "APP_EXCEPTION_CODES_V17" 
AS
...
CREATE OR REPLACE PACKAGE "GLOBAL_CONSTANTS_V17" AS 
...
CREATE OR REPLACE PACKAGE "GLOBAL_ENUMS_V17" AS 
...
CREATE OR REPLACE PACKAGE "GLOBAL_STATUSES_V17" AS
...
BEGIN
  PROCS_COMMON_V17.ISO8601DURATION_TO_INTERVALS(in_duration, var_years, var_months, var_days);
...
BEGIN
  IF in_error_stacktrace IS NOT NULL THEN
    RETURN in_sproc_name || ': ' || in_error_message || ': ' || GLOBAL_CONSTANTS_V17.EXCEPTION_DELIMETER || in_error_stacktrace;
  END IF;
  RETURN
...
grant execute on CORE_OWNER.APP_EXCEPTION_CODES_V17 to core_hist_owner;
grant execute on CORE_OWNER.APP_EXCEPTION_CODES_V17 to ops_owner;
grant execute on CORE_OWNER.APP_EXCEPTION_CODES_V17 to ops_hist_owner;

grant execute on CORE_OWNER.GLOBAL_ENUMS_V17 to core_hist_owner;
grant execute on CORE_OWNER.GLOBAL_ENUMS_V17 to ops_owner;
grant execute on CORE_OWNER.GLOBAL_ENUMS_V17 to ops_hist_owner;

grant execute on CORE_OWNER.GLOBAL_STATUSES_V17 to core_hist_owner;
grant execute on CORE_OWNER.GLOBAL_STATUSES_V17 to ops_owner;
grant execute on CORE_OWNER.GLOBAL_STATUSES_V17 to ops_hist_owner;
