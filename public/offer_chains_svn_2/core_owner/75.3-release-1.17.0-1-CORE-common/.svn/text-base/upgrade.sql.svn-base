...
CREATE OR REPLACE PACKAGE "APP_EXCEPTION_CODES_V23" 
AS
...
CREATE OR REPLACE PACKAGE "GLOBAL_CONSTANTS_V23" AS 
...
CREATE OR REPLACE PACKAGE "GLOBAL_ENUMS_V23" AS 
...
CREATE OR REPLACE PACKAGE "GLOBAL_STATUSES_V23" AS
...
CREATE OR REPLACE PACKAGE "NOTIFICATION_STATUSES_V23" AS
...
CREATE OR REPLACE PACKAGE "PROCS_COMMON_V23" AS 
...
END PROCS_COMMON_V23;
.
/

CREATE OR REPLACE PACKAGE BODY "PROCS_COMMON_V23" AS
...
BEGIN
  PROCS_COMMON_V23.ISO8601DURATION_TO_INTERVALS(in_duration, var_years, var_months, var_days);
...
BEGIN
  IF in_error_stacktrace IS NOT NULL THEN
    RETURN in_sproc_name || ': ' || in_error_message || ': ' || GLOBAL_CONSTANTS_V23.EXCEPTION_DELIMETER || in_error_stacktrace;
  END IF;
  RETURN
