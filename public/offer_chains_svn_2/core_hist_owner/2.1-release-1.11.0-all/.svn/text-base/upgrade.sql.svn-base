...
*/
  ...
  in_transaction_amount        IN NUMBER,
  in_created_by                IN VARCHAR2,
  in_create_date               IN DATE,
  in_system_activity_reason_id IN NUMBER
);
...
/*
Throws exceptions:
APP_EXCEPTION_CODES_V14.UNKNOWN_ERROR
*/
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE_APPLICATION_ERROR(CORE_OWNER.APP_EXCEPTION_CODES_V14.UNKNOWN_ERROR,
      SPROC_NAME||': Unknown error: '||SQLERRM);
...
CREATE OR REPLACE PACKAGE "PUBLIC_PROCS_CORE_V16" AS 
...
END PUBLIC_PROCS_CORE_V16;
.
/

CREATE OR REPLACE PACKAGE BODY "PUBLIC_PROCS_CORE_V16" AS
...
BEGIN
  PROCS_HISTORY_V16.CREATE_ITUNES_RECEIPT_HISTORY (
    in_id => in_id,
...
CREATE OR REPLACE PACKAGE "PROCS_PARTITIONS_V16" AS 
...
END PROCS_PARTITIONS_V16;
.
/

CREATE OR REPLACE PACKAGE BODY "PROCS_PARTITIONS_V16" AS
...
grant execute on PUBLIC_PROCS_CORE_V16 to core_owner;
grant execute on PUBLIC_PROCS_CORE_V16 to core_app;
