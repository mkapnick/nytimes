...
PROCEDURE CREATE_AASR_HISTORY(
  in_aasr_id                   IN AMAZON_APPSTORE_RECEIPT_HIST.AASR_ID%TYPE,
  in_subscription_id           IN AMAZON_APPSTORE_RECEIPT_HIST.SUBSCRIPTION_ID%TYPE,
  in_user_id                   IN AMAZON_APPSTORE_RECEIPT_HIST.USER_ID%TYPE,
  in_item_type                 IN AMAZON_APPSTORE_RECEIPT_HIST.ITEM_TYPE%TYPE,
  in_start_date                IN AMAZON_APPSTORE_RECEIPT_HIST.START_DATE%TYPE,
  in_end_date                  IN AMAZON_APPSTORE_RECEIPT_HIST.END_DATE%TYPE,
  in_sku                       IN AMAZON_APPSTORE_RECEIPT_HIST.SKU%TYPE,
  in_purchase_token            IN AMAZON_APPSTORE_RECEIPT_HIST.PURCHASE_TOKEN%TYPE,
  in_aasr_create_date          IN AMAZON_APPSTORE_RECEIPT_HIST.AASR_CREATE_DATE%TYPE,
  in_aasr_created_by           IN AMAZON_APPSTORE_RECEIPT_HIST.AASR_CREATED_BY%TYPE,
  in_aasr_update_date          IN AMAZON_APPSTORE_RECEIPT_HIST.AASR_UPDATE_DATE%TYPE,
  in_aasr_updated_by           IN AMAZON_APPSTORE_RECEIPT_HIST.AASR_UPDATED_BY%TYPE,
  in_system_activity_reason_id IN AMAZON_APPSTORE_RECEIPT_HIST.SYSTEM_ACTIVITY_REASON_ID%TYPE,
  in_created_by                IN AMAZON_APPSTORE_RECEIPT_HIST.CREATED_BY%TYPE,
  in_last_check_date           IN AMAZON_APPSTORE_RECEIPT_HIST.LAST_CHECK_DATE%TYPE
);
...
var_id AMAZON_APPSTORE_RECEIPT_HIST.ID%TYPE := AASR_HIST_ID_SEQ.nextval;
var_now AMAZON_APPSTORE_RECEIPT_HIST.CREATE_DATE%TYPE := SYSDATE;
BEGIN
  INSERT INTO AMAZON_APPSTORE_RECEIPT_HIST (
    ID,
    AASR_ID,
    SUBSCRIPTION_ID,
    USER_ID,
...
EXCEPTION
WHEN OTHERS THEN
  RAISE_APPLICATION_ERROR(CORE_OWNER.APP_EXCEPTION_CODES_V23.UNKNOWN_ERROR,
...
CREATE OR REPLACE PACKAGE "PUBLIC_PROCS_CORE_V23" AS
...
END PUBLIC_PROCS_CORE_V23;
.
/

CREATE OR REPLACE PACKAGE BODY "PUBLIC_PROCS_CORE_V23" AS
...
AS
BEGIN
  PROCS_HISTORY_V23.CREATE_AASR_HISTORY(
      in_aasr_id                   => in_aasr_id,
      ...
      in_aasr_create_date          => in_aasr_create_date,
      in_aasr_created_by           => in_aasr_created_by,
      in_aasr_update_date          => in_aasr_update_date,
      in_aasr_updated_by           => in_aasr_updated_by,
      in_system_activity_reason_id => in_system_activity_reason_id,
...
BEGIN
  PROCS_HISTORY_V23.CREATE_ITUNES_RECEIPT_HISTORY (
    in_id => in_id,
    in_subscription_id => in_subscription_id,
...
BEGIN
  PROCS_HISTORY_V23.CREATE_NOTIFICATION_HISTORY(
    in_notification_id,
    in_account_id,
    in_group_id,
    notification_reason_type_id,
...
BEGIN
  PROCS_HISTORY_V23.CREATE_NOTIF_FAILURE_HISTORY(
    in_error_message,
    in_notification_id,
    in_create_date
);
...
CREATE OR REPLACE PACKAGE "PROCS_PARTITIONS_V23" AS 
...
END PROCS_PARTITIONS_V23;
.
/

CREATE OR REPLACE PACKAGE BODY "PROCS_PARTITIONS_V23" AS
...
grant execute on PUBLIC_PROCS_CORE_V23 to core_owner;
grant execute on PUBLIC_PROCS_CORE_V23 to core_app;
grant execute on PUBLIC_PROCS_CORE_V23 to ops_notif_app;
