...
EXCEPTION
WHEN OTHERS THEN
  RAISE_APPLICATION_ERROR(CORE_OWNER.APP_EXCEPTION_CODES_V22.UNKNOWN_ERROR,
...
CREATE OR REPLACE PACKAGE "PUBLIC_PROCS_CORE_V22" AS
...
END PUBLIC_PROCS_CORE_V22;
.
/

CREATE OR REPLACE PACKAGE BODY "PUBLIC_PROCS_CORE_V22" AS
...
AS
BEGIN
  PROCS_HISTORY_V22.CREATE_AASR_HISTORY(
...
BEGIN
  PROCS_HISTORY_V22.CREATE_ITUNES_RECEIPT_HISTORY (
    in_id => in_id,
    in_subscription_id => in_subscription_id,
...
BEGIN
  PROCS_HISTORY_V22.CREATE_NOTIFICATION_HISTORY(
    in_notification_id,
    in_account_id,
    in_group_id,
    notification_reason_type_id,
...
BEGIN
  PROCS_HISTORY_V22.CREATE_NOTIF_FAILURE_HISTORY(
    in_error_message,
    in_notification_id,
    in_create_date
);
...
CREATE OR REPLACE PACKAGE "PROCS_PARTITIONS_V22" AS 
...
END PROCS_PARTITIONS_V22;
.
/

CREATE OR REPLACE PACKAGE BODY "PROCS_PARTITIONS_V22" AS
...
grant execute on PUBLIC_PROCS_CORE_V22 to core_owner;
grant execute on PUBLIC_PROCS_CORE_V22 to core_app;
grant execute on PUBLIC_PROCS_CORE_V22 to ops_notif_app;
