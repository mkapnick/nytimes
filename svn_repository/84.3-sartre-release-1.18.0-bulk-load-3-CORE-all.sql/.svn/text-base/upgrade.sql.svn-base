prompt Compiling header for notification_statuses.sql

.
/

prompt Compiling header for procs_account.sql
CREATE OR REPLACE PACKAGE "PROCS_ACCOUNT_V24" AS
...
PROCEDURE CREATE_ACTIVE_ACCOUNT(
  ...
  out_new_account_id OUT ACCOUNT.ID%TYPE
);
...
PROCEDURE GET_ACCOUNT_CREDIT_CARDS (
  in_group_id    IN ACCOUNT.GROUP_ID%TYPE,
  in_status_id   IN CREDIT_CARD.CREDIT_CARD_STATUS_ID%TYPE DEFAULT GLOBAL_STATUSES_V24.CREDIT_CARD_ACTIVE,
  out_result_set OUT SYS_REFCURSOR
);
...
PROCEDURE GET_ACCOUNT_GIFT_CERTIFICATES (
  ...
  in_instr_status   IN NUMBER DEFAULT GLOBAL_CONSTANTS_V24.TRUE
);
...
PROCEDURE GET_ACCOUNT_PAYPALS(
  in_group_id    IN  ACCOUNT.GROUP_ID%TYPE,
  in_status_id   IN  PAYPAL.PAYPAL_STATUS_ID%TYPE DEFAULT GLOBAL_STATUSES_V24.PAYPAL_ACTIVE,
  out_result_set OUT SYS_REFCURSOR
);
...
END PROCS_ACCOUNT_V24;
.
/

prompt Compiling header for procs_account_cru.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_ACCOUNT_CRU
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_ACCOUNT_CRU_V24" AS 
...
END PROCS_ACCOUNT_CRU_V24;
.
/

prompt Compiling header for procs_address.sql
CREATE OR REPLACE PACKAGE "PROCS_ADDRESS_V24" AS

PROCEDURE CREATE_ADDRESS(
/*
Throws exceptions:
APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR
*/
...
END PROCS_ADDRESS_V24;
.
/

prompt Compiling header for procs_address_cru.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_ADDRESS_CRU
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_ADDRESS_CRU_V24" AS 
...
END PROCS_ADDRESS_CRU_V24;
.
/

prompt Compiling header for procs_adjustments.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_ADJUSTMENTS
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_ADJUSTMENTS_V24" AS 
...
END PROCS_ADJUSTMENTS_V24;
.
/

prompt Compiling header for procs_adx.sql
CREATE OR REPLACE PACKAGE "PROCS_ADX_V24" AS
...
prompt Compiling header for procs_amazon.sql
CREATE OR REPLACE PACKAGE "PROCS_AMAZON_V24" AS

PROCEDURE AASR_PURCHASE_TOKEN_USED(
  in_purchase_token IN AMAZON_APPSTORE_RECEIPT.PURCHASE_TOKEN%TYPE,
  out_data          OUT NUMBER
);

PROCEDURE UPDATE_AASR_LAST_CHECK_DATE(
  in_id         IN AMAZON_APPSTORE_RECEIPT.ID%TYPE,
  in_updated_by IN AMAZON_APPSTORE_RECEIPT.UPDATED_BY%TYPE
);

PROCEDURE EXPIRED_AASR_SUB_IDS(
  out_data   OUT SYS_REFCURSOR
);

PROCEDURE UPDATE_AASR_PURCHASE_TOKEN(
  in_id             IN AMAZON_APPSTORE_RECEIPT.ID%TYPE,
  in_purchase_token IN AMAZON_APPSTORE_RECEIPT.PURCHASE_TOKEN%TYPE,
  in_updated_by     IN AMAZON_APPSTORE_RECEIPT.UPDATED_BY%TYPE
);

PROCEDURE UPDATE_AASR_END_DATE(
  in_id         IN AMAZON_APPSTORE_RECEIPT.ID%TYPE,
  in_end_date   IN AMAZON_APPSTORE_RECEIPT.END_DATE%TYPE,
  in_updated_by IN AMAZON_APPSTORE_RECEIPT.UPDATED_BY%TYPE
);

PROCEDURE AASR_RECEIPTS_WITH_ACTIVE_SUBS(
  out_data   OUT SYS_REFCURSOR,
  in_process_name IN PROCESS_RETRY_THROTTLE.PROCESS_NAME%TYPE
);

PROCEDURE ADD_AMAZON_APPSTORE_RECEIPT(
  in_subscription_id IN AMAZON_APPSTORE_RECEIPT.SUBSCRIPTION_ID%TYPE,
  in_user_id         IN AMAZON_APPSTORE_RECEIPT.USER_ID%TYPE,
  in_item_type       IN AMAZON_APPSTORE_RECEIPT.ITEM_TYPE%TYPE,
  in_start_date      IN AMAZON_APPSTORE_RECEIPT.START_DATE%TYPE,
  in_end_date        IN AMAZON_APPSTORE_RECEIPT.END_DATE%TYPE DEFAULT NULL,
  in_sku             IN AMAZON_APPSTORE_RECEIPT.SKU%TYPE,
  in_purchase_token  IN AMAZON_APPSTORE_RECEIPT.PURCHASE_TOKEN%TYPE,
  in_created_by      IN AMAZON_APPSTORE_RECEIPT.CREATED_BY%TYPE
);

PROCEDURE AMAZON_APPSTORE_RECEIPTS(
  in_user_id IN AMAZON_APPSTORE_RECEIPT.USER_ID%TYPE,
  in_sku     IN AMAZON_APPSTORE_RECEIPT.SKU%TYPE,
  out_data   OUT SYS_REFCURSOR
);
...
END PROCS_AMAZON_V24;
.
/

prompt Compiling header for procs_amazon_cru.sql
CREATE OR REPLACE PACKAGE "PROCS_AMAZON_CRU_V24" AS

PROCEDURE UPDATE_AMAZON_APPSTORE_RECEIPT(
  in_id              IN AMAZON_APPSTORE_RECEIPT.ID%TYPE,
  in_updated_by      IN AMAZON_APPSTORE_RECEIPT.UPDATED_BY%TYPE,
  in_subscription_id IN AMAZON_APPSTORE_RECEIPT.SUBSCRIPTION_ID%TYPE DEFAULT NULL,
  in_user_id         IN AMAZON_APPSTORE_RECEIPT.USER_ID%TYPE DEFAULT NULL,
  in_item_type       IN AMAZON_APPSTORE_RECEIPT.ITEM_TYPE%TYPE DEFAULT NULL,
  in_start_date      IN AMAZON_APPSTORE_RECEIPT.START_DATE%TYPE DEFAULT NULL,
  in_end_date        IN AMAZON_APPSTORE_RECEIPT.END_DATE%TYPE DEFAULT NULL,
  in_sku             IN AMAZON_APPSTORE_RECEIPT.SKU%TYPE DEFAULT NULL,
  in_purchase_token  IN AMAZON_APPSTORE_RECEIPT.PURCHASE_TOKEN%TYPE DEFAULT NULL,
  in_last_check_date IN AMAZON_APPSTORE_RECEIPT.LAST_CHECK_DATE%TYPE DEFAULT NULL
);
...
END PROCS_AMAZON_CRU_V24;
.
/

prompt Compiling header for procs_ar_bal_applies.sql


CREATE OR REPLACE PACKAGE "PROCS_AR_BAL_APPLIES_V24" AS


PROCEDURE APPLY_BALANCE (
  in_invoice_id  IN AR_BALANCE_APPLIES.INVOICE_ID%TYPE,
  in_charge_id IN AR_BALANCE_APPLIES.CHARGE_ID%TYPE,
  in_amount IN AR_BALANCE_APPLIES.AMOUNT%TYPE,
  in_created_by       IN AR_BALANCE_APPLIES.CREATED_BY%TYPE
);


END PROCS_AR_BAL_APPLIES_V24;
.
/

prompt Compiling header for procs_charge.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_CHARGE
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_CHARGE_V24" AS 

PROCEDURE CREATE_CHARGE(
/*
Throws exceptions:
APP_EXCEPTION_CODES_V24.NOT_FOUND
APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR
*/
  in_invoice_id         IN NUMBER,
...
END PROCS_CHARGE_V24;
.
/

prompt Compiling header for procs_charge_cru.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_CHARGE_CRU
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_CHARGE_CRU_V24" AS 
...
END PROCS_CHARGE_CRU_V24;
.
/

prompt Compiling header for procs_cupy.sql
CREATE OR REPLACE PACKAGE "PROCS_CUPY" AS
  ...
  PROCEDURE GET_CREDIT_CARD_INFO (
    ...
    out_card_info        OUT SYS_REFCURSOR
  );
...
prompt Compiling header for procs_entitlement.sql
CREATE OR REPLACE
PACKAGE PROCS_ENTITLEMENT_V24 AS
...
END PROCS_ENTITLEMENT_V24;
.
/

prompt Compiling header for procs_fin_instruments.sql
CREATE OR REPLACE PACKAGE "PROCS_FIN_INSTRUMENTS_V24" AS
...
PROCEDURE START_GC_PURCHASING (
  ...
  in_expiration_date        IN DATE,
  in_campaign               IN VARCHAR2,
  in_reference_code         IN VARCHAR2,
...
PROCEDURE GET_EXPIRING_PAYPAL (
  in_expire_window_days   IN NUMBER,
  in_creation_limit_days  IN NUMBER,
  in_retry_throttle_name  PROCESS_RETRY_THROTTLE.PROCESS_NAME%TYPE,
  out_result_set          OUT SYS_REFCURSOR
);
...
END PROCS_FIN_INSTRUMENTS_V24;
.
/

prompt Compiling header for procs_fin_instruments_cru.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_FIN_INSTRUMENTS_CRU
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_FIN_INSTRUMENTS_CRU_V24" AS 
...
PROCEDURE CREATE_GIFT_CERTIFICATE(
  ...
  in_campaign                   IN GC_CAMPAIGN_AND_REF.CAMPAIGN%TYPE DEFAULT NULL,
  in_reference_code             IN GC_CAMPAIGN_AND_REF.REFERENCE_CODE%TYPE DEFAULT NULL
);
...
END PROCS_FIN_INSTRUMENTS_CRU_V24;
.
/

prompt Compiling header for procs_group_account.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_GROUP_ACCOUNT
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_GROUP_ACCOUNT_V24" As
...
END PROCS_GROUP_ACCOUNT_V24;
.
/

prompt Compiling header for procs_group_account_cru.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_GROUP_ACCOUNT
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_GROUP_ACCOUNT_CRU_V24" AS
...
END PROCS_GROUP_ACCOUNT_CRU_V24;
.
/

prompt Compiling header for procs_history.sql
CREATE OR REPLACE PACKAGE "PROCS_HISTORY_V24" AS

PROCEDURE CREATE_AASR_HISTORY(
  in_id                        IN CORE_OWNER.AMAZON_APPSTORE_RECEIPT.ID%TYPE,
...
END PROCS_HISTORY_V24;
.
/

prompt Compiling header for procs_invoice.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_INVOICE
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_INVOICE_V24" AS
...
APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR
Return:
  GLOBAL_CONSTANTS_V24.TRUE if ACCOUNT.EXEMPT_ID is not null
  GLOBAL_CONSTANTS_V24.FALSE else
*/
  in_invoice_id     IN NUMBER,
  out_is_tax_exempt OUT NUMBER
);
...
END PROCS_INVOICE_V24;
.
/

prompt Compiling header for procs_invoice_cru.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_INVOICE_CRU
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_INVOICE_CRU_V24" AS 
...
END PROCS_INVOICE_CRU_V24;
.
/

prompt Compiling header for procs_itunes_receipt.sql
/*
...
CREATE OR REPLACE PACKAGE "PROCS_ITUNES_RECEIPT_V24" AS
...
END PROCS_ITUNES_RECEIPT_V24;
.
/

prompt Compiling header for procs_itunes_receipt_cru.sql
/*
CREATE TABLE ITUNES_RECEIPT (
  id NUMBER NOT NULL ENABLE,
  subscription_id NUMBER NOT NULL ENABLE,
...
CREATE OR REPLACE PACKAGE "PROCS_ITUNES_RECEIPT_CRU_V24" AS
...
END PROCS_ITUNES_RECEIPT_CRU_V24;
.
/

prompt Compiling header for procs_license.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_LICENSE
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_LICENSE_V24" AS 
...
PROCEDURE UPDATE_LICENSE_STATUS(
    ...
    in_updated_by     IN VARCHAR2,
    in_ent_end        IN NUMBER DEFAULT GLOBAL_CONSTANTS_V24.FALSE
);
...
END PROCS_LICENSE_V24;
.
/

prompt Compiling header for procs_license_cru.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_LICENSE_CRU
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_LICENSE_CRU_V24" AS 
...
END PROCS_LICENSE_CRU_V24;
.
/

prompt Compiling header for procs_line_items.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_LINE_ITEMS
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_LINE_ITEMS_V24" AS 
...
END PROCS_LINE_ITEMS_V24;
.
/

prompt Compiling header for procs_line_items_cru.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_LINE_ITEMS_CRU
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_LINE_ITEMS_CRU_V24" AS 
...
END PROCS_LINE_ITEMS_CRU_V24;
.
/

prompt Compiling header for procs_locking.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_LOCKING
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_LOCKING_V24" AS 
...
END PROCS_LOCKING_V24;
.
/

prompt Compiling header for procs_notification.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_NOTIFICATION
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_NOTIFICATION_V24" AS
...
PROCEDURE ADD_SINGLE_NOTIFICATION_ROW (
  in_sender_account_id     IN NUMBER DEFAULT 0,
  in_recipient_group_id    IN NUMBER,
  in_notification_type_id  IN NUMBER,
  in_date_to_notify        IN DATE,
...
END PROCS_NOTIFICATION_V24;
.
/

prompt Compiling header for procs_offer_chain.sql
CREATE OR REPLACE PACKAGE "PROCS_OFFER_CHAIN_V24" AS

PROCEDURE IS_GC_OFFER_CHAIN(
  in_offer_chain_id IN OFFER_CHAIN.ID%TYPE,
  out_data OUT NUMBER
);

PROCEDURE OC_ID_BY_ITUNES_PRODUCT_ID(
  in_itunes_product_id IN  OFFER_CHAIN.ITUNES_PRODUCT_ID%TYPE,
  out_data             OUT OFFER_CHAIN.ID%TYPE
);

PROCEDURE OFFER_CHAIN_ID_BY_AMAZON_SKU(
  in_amazon_appstore_sku IN  OFFER_CHAIN.AMAZON_APPSTORE_SKU%TYPE,
  out_data               OUT OFFER_CHAIN.ID%TYPE
);
...
FUNCTION CHECK_FOR_SAME_PRODUCTS (
  ...
  in_use_eligibility_rules IN NUMBER DEFAULT GLOBAL_CONSTANTS_V24.FALSE
) RETURN NUMBER;
...
END PROCS_OFFER_CHAIN_V24;
.
/

prompt Compiling header for procs_polling_sync.sql
...
prompt Compiling header for procs_process_retry.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_PROCESS_RETRY_THROTTLE
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_PROCESS_RETRY_V24" AS
...
END PROCS_PROCESS_RETRY_V24;
.
/

prompt Compiling header for procs_product.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_PRODUCT
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_PRODUCT_V24" AS 
...
END PROCS_PRODUCT_V24;
.
/

prompt Compiling header for procs_reconciliation_cru.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_RECONCILIATION_CRU
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_RECONCILIATION_CRU_V24" AS 
...
END PROCS_RECONCILIATION_CRU_V24;
.
/

prompt Compiling header for procs_reporting.sql
...
prompt Compiling header for procs_reports.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_REPORTS
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_REPORTS_V24" AS 
...
END PROCS_REPORTS_V24;
.
/

prompt Compiling header for procs_subscription.sql
CREATE OR REPLACE PACKAGE "PROCS_SUBSCRIPTION_V24" AS
...
PROCEDURE START_SUBSCRIPTION_CREATION (
  ...
  in_check_dupe_products   IN NUMBER DEFAULT GLOBAL_CONSTANTS_V24.TRUE
);
...
PROCEDURE CANCEL_SUBSCRIPTION_INVOICE (
  in_invoice_id        IN NUMBER,
  in_updated_by        IN VARCHAR2,
  in_refundable        IN NUMBER DEFAULT GLOBAL_CONSTANTS_V24.FALSE
);
...
PROCEDURE FINALIZE_CANCELATION (
  ...
  in_agent_id           IN NUMBER,
  in_updated_by         IN VARCHAR2,
  in_revoke_entitlements IN NUMBER DEFAULT GLOBAL_CONSTANTS_V22.FALSE
);
...
END PROCS_SUBSCRIPTION_V24;
.
/

prompt Compiling header for procs_subscription_cru.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_SUBSCRIPTION_CRU
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_SUBSCRIPTION_CRU_V24" AS 
...
END PROCS_SUBSCRIPTION_CRU_V24;
.
/

prompt Compiling header for procs_system.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_SYSTEM
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_SYSTEM_V24" AS 
...
END PROCS_SYSTEM_V24;
.
/

prompt Compiling header for procs_taxes.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_TAXES
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_TAXES_V24" AS 
...
PROCEDURE CHECK_COUNTRY_FOR_EXCLUSION (
  ...
  out_is_founded  OUT NUMBER -- GLOBAL_CONSTANT.TRUE of GLOBAL_CONSTANTS_V24.FALSE
);
...
prompt Compiling header for procs_taxes_cru.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_TAXES_CRU
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_TAXES_CRU_V24" AS 
...
END PROCS_TAXES_CRU_V24;
.
/

prompt Compiling header for procs_test.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_TEST
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_TEST_V24" AS 
...
prompt Compiling header for procs_transaction.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_TRANSACTION
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_TRANSACTION_V24" AS 
...
*/
  ...
  in_created_by             IN VARCHAR2,
  in_order_id               IN VARCHAR2,
  in_is_refund              IN NUMBER DEFAULT GLOBAL_CONSTANTS_V24.FALSE,
...
Throws exceptions:
APP_EXCEPTION_CODES_V24.NOT_FOUND
APP_EXCEPTION_CODES_V24.INTRNAL_ERROR
APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR
*/
...
END PROCS_TRANSACTION_V24;
.
/

prompt Compiling header for procs_transaction_cru.sql
--------------------------------------------------------------------------------
-- DDL for package PROCS_TRANSACTION_CRU
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PROCS_TRANSACTION_CRU_V24" AS 
...
PROCEDURE CREATE_TRANSACTION (
  ...
  in_order_id              IN TRANSACTION.ORDER_ID%TYPE,
  in_is_refund             IN TRANSACTION.IS_REFUND%TYPE DEFAULT GLOBAL_CONSTANTS_V24.FALSE,
...
END PROCS_TRANSACTION_CRU_V24;
.
/

prompt Compiling header for public_procs_billing.sql
--------------------------------------------------------------------------------
-- DDL for package PUBLIC_PROCS_BILLING
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PUBLIC_PROCS_BILLING_V24" AS 
...
Throws exceptions:
APP_EXCEPTION_CODES_V24.NOT_FOUND
APP_EXCEPTION_CODES_V24.INTERNAL_ERROR
APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR
*/
    in_subs_id    IN NUMBER ,
    in_updated_by IN VARCHAR2
...
END PUBLIC_PROCS_BILLING_V24;
.
/

prompt Compiling header for public_procs_client.sql
--------------------------------------------------------------------------------
-- DDL for package PUBLIC_PROCS_CLIENT
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PUBLIC_PROCS_CLIENT_V24" AS 

PROCEDURE GET_NOTIFICATION_TYPE_BY_NAME (
/*
Throws exceptions:
CORE_OWNER.APP_EXCEPTION_CODES_V24.NOT_FOUND
CORE_OWNER.APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR
*/
  in_notification_type_name IN VARCHAR2,
  out_notification_type_id  OUT NUMBER
);
...
END PUBLIC_PROCS_CLIENT_V24;
.
/

prompt Compiling header for public_procs_notification.sql
--------------------------------------------------------------------------------
-- DDL for package PUBLIC_PROCS_NOTIFICATION
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PUBLIC_PROCS_NOTIFICATION_V24" AS 
...
END PUBLIC_PROCS_NOTIFICATION_V24;
.
/

prompt Compiling header for public_procs_renewal.sql
--------------------------------------------------------------------------------
-- DDL for package PUBLIC_PROCS_RENEWAL
--------------------------------------------------------------------------------

CREATE OR REPLACE PACKAGE "PUBLIC_PROCS_RENEWAL_V24" AS
...
PROCEDURE GET_EXPIRING_PAYPAL (
  in_expire_window_days   IN NUMBER,
  in_creation_limit_days  IN NUMBER,
  in_retry_throttle_name  IN PROCESS_RETRY_THROTTLE.PROCESS_NAME%TYPE,
  out_result_set          OUT SYS_REFCURSOR
);

END PUBLIC_PROCS_RENEWAL_V24;
.
/

prompt Compiling body for procs_account_cru.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_ACCOUNT_CRU_V24" AS
...
BEGIN
  ...
  -- CREATE HISTORY
  PROCS_HISTORY_V24.CREATE_ACCOUNT_HISTORY(
    in_account_id                => in_account_id,
    in_system_activity_reason_id => GLOBAL_ENUMS_V24.SAC_SYSTEM_APPLIED_RULE
  );
...
prompt Compiling body for procs_address_cru.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_ADDRESS_CRU_V24" AS
...
BEGIN
  ...
  -- Create history
  PROCS_HISTORY_V24.CREATE_ADDRESS_HISTORY(
...
prompt Compiling body for procs_amazon_cru.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_AMAZON_CRU_V24" AS
...
SPROC_NAME CONSTANT VARCHAR2(32) := 'UPDATE_AMAZON_APPSTORE_RECEIPT';
BEGIN
  CORE_OWNER.PROCS_HISTORY_V24.CREATE_AASR_HISTORY(
      in_id => in_id,
      in_system_activity_reason_id => GLOBAL_ENUMS_V24.SAC_SYSTEM_APPLIED_RULE,
      in_created_by => in_updated_by
  );

  UPDATE AMAZON_APPSTORE_RECEIPT SET
    SUBSCRIPTION_ID = NVL(in_subscription_id,SUBSCRIPTION_ID),
    USER_ID         = NVL(in_user_id,USER_ID),
    ITEM_TYPE       = NVL(in_item_type,ITEM_TYPE),
    START_DATE      = NVL(in_start_date,START_DATE),
    END_DATE        = NVL(in_end_date,END_DATE),
    SKU             = NVL(in_sku,SKU),
    PURCHASE_TOKEN  = NVL(in_purchase_token,PURCHASE_TOKEN),
    LAST_CHECK_DATE = NVL(in_last_check_date,LAST_CHECK_DATE),
    UPDATE_DATE     = SYSDATE,
    UPDATED_BY      = in_updated_by
  WHERE
    ID = in_id;
END UPDATE_AMAZON_APPSTORE_RECEIPT;
...
SPROC_NAME CONSTANT VARCHAR2(32) := 'ADD_AMAZON_APPSTORE_RECEIPT';
var_now    DATE := SYSDATE;
BEGIN
  INSERT INTO CORE_OWNER.AMAZON_APPSTORE_RECEIPT (
    ID,
    SUBSCRIPTION_ID,
    USER_ID,
    ITEM_TYPE,
    ...
    CORE_OWNER.AAS_RECEIPT_ID_SEQ.nextval,
    in_subscription_id,
    in_user_id,
    in_item_type,
    in_start_date,
...
END ADD_AMAZON_APPSTORE_RECEIPT;
...
BEGIN
  ...
  WHERE
    ...
    and s.subscription_status_id = GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE;
...
WHEN AMAZON_SUB_USED THEN
  PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.DUPLICATE_ERROR,
    SPROC_NAME, 'Amazon sub already used', SQLERRM);
WHEN DUP_VAL_ON_INDEX THEN
  PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.DUPLICATE_ERROR,
    SPROC_NAME, 'Duplicate value', SQLERRM);
WHEN OTHERS THEN
  PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR,
...
prompt Compiling body for procs_charge_cru.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_CHARGE_CRU_V24" AS
...
BEGIN
  ...
  INSERT INTO
    ...
    VALUES
    (
        CHARGE_STATUS_LOG_ID_SEQ.nextval,
        var_charge_id,
        in_charge_status_id,
        sysdate,
        in_created_by
...
BEGIN
  ...
  -- Create history
  PROCS_HISTORY_V24.CREATE_CHARGE_HISTORY(
...
prompt Compiling body for procs_fin_instruments_cru.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_FIN_INSTRUMENTS_CRU_V24" AS
...
BEGIN
  ...
  -- Create history
  PROCS_HISTORY_V24.CREATE_CREDIT_CARD_HISTORY(
...
BEGIN
  -- Create history
  PROCS_HISTORY_V24.CREATE_PAYPAL_HISTORY(
...
BEGIN
  ...
  INSERT INTO GIFT_CERTIFICATE (
    ...
    IF in_campaign IS NOT NULL THEN
      INSERT INTO GC_CAMPAIGN_AND_REF(GC_ID, CAMPAIGN, REFERENCE_CODE)
        VALUES(var_gift_certificate_id, in_campaign, in_reference_code);
    END IF;
...
BEGIN
  ...
  -- Create history
  PROCS_HISTORY_V24.CREATE_GIFT_CERT_HISTORY(
...
prompt Compiling body for procs_group_account_cru.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_GROUP_ACCOUNT_CRU_V24" AS
...
EXCEPTION
  WHEN OTHERS THEN
    Procs_Common_V24.Throw_Exception(APP_EXCEPTION_CODES_V24.Internal_Error,
...
BEGIN
  ...
  set
    IR.GROUP_ACC_IP_RNG_STATUS_ID = GLOBAL_STATUSES_V24.GROUP_ACC_IP_RNG_INACTIVE,
    IR.UPDATED_BY = in_updated_by,
    IR.UPDATE_DATE = sysdate
  where
    IR.GROUP_ACCOUNT_ID = in_group_account_id and
    IR.GROUP_ACC_IP_RNG_STATUS_ID = GLOBAL_STATUSES_V24.GROUP_ACC_IP_RNG_ACTIVE
  ;
EXCEPTION
WHEN OTHERS THEN
  PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR,
...
WHEN NO_DATA_FOUND THEN
  PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.INVALID_PARAMETER,
    SPROC_NAME, 'Missing or invalid group_account_ip_range_id '||in_id, SQLERRM);
WHEN OTHERS THEN
  PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR,
...
BEGIN
    ...
    VALUES (
      ...
      in_created_by,
      sysdate,
      GLOBAL_STATUSES_V24.GROUP_ACC_IP_RNG_ACTIVE
    );
EXCEPTION
WHEN OTHERS THEN
  PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR,
...
BEGIN
  ...
  set
    ED.IS_ACTIVE = GLOBAL_STATUSES_V24.GROUP_ACC_EMAIL_DOMAIN_INACT,
    ED.UPDATED_BY = in_updated_by,
    ED.UPDATE_DATE = sysdate
  where
    ED.GROUP_ACCOUNT_ID = in_group_account_id and
    ED.IS_ACTIVE = GLOBAL_STATUSES_V24.GROUP_ACC_EMAIL_DOMAIN_ACT
  ;
EXCEPTION
WHEN OTHERS THEN
  PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR,
...
BEGIN
  ...
  if(sql%rowcount = 0) then
    PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.NOT_FOUND,
...
BEGIN
  ...
  PROCS_SUBSCRIPTION_V24.ANNOTATE_SUBSCRIPTION(
...
prompt Compiling body for procs_invoice_cru.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_INVOICE_CRU_V24" AS
...
BEGIN
  -- Create history
  PROCS_HISTORY_V24.CREATE_INVOICE_HISTORY(
...
prompt Compiling body for procs_itunes_receipt_cru.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_ITUNES_RECEIPT_CRU_V24" AS
...
BEGIN
  ...
  WHERE
    ...
    S.SUBSCRIPTION_STATUS_ID = GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE;
...
BEGIN
  ...
  FOR REC IN (SELECT * FROM CORE_OWNER.ITUNES_RECEIPT WHERE ID = in_id) LOOP
    CORE_HIST_OWNER.PUBLIC_PROCS_CORE_V24.CREATE_ITUNES_RECEIPT_HISTORY
    (
        rec.id,
        rec.subscription_id,
        rec.receipt,
...
prompt Compiling body for procs_license_cru.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_LICENSE_CRU_V24" AS
...
BEGIN
  -- Create history
  PROCS_HISTORY_V24.CREATE_LICENSE_HISTORY(
...
prompt Compiling body for procs_line_items_cru.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_LINE_ITEMS_CRU_V24" AS
...
prompt Compiling body for procs_reconciliation_cru.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_RECONCILIATION_CRU_V24" AS
...
BEGIN
  ...
  INSERT INTO
   ...
   BEGIN		
	SELECT
		c.ID into var_charge_id
	FROM
		CHARGE c, TRANSACTION t
	WHERE
		c.TRANSACTION_ID = t.ID
		AND T.ORDER_ID = LOWER(in_ind_ref_number)
		AND rownum <= 1;
	EXCEPTION 
		WHEN OTHERS THEN
			var_charge_id := NULL;
    ...
    IF var_charge_id IS NOT NULL
    THEN
        PROCS_CHARGE_V24.UPDATE_CHARGE_STATUS(var_charge_id,GLOBAL_STATUSES_V24.CHARGEBACK_SETTLED,in_created_by);
    END IF;
...
prompt Compiling body for procs_subscription_cru.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_SUBSCRIPTION_CRU_V24" AS
...
BEGIN
  -- Create history
  PROCS_HISTORY_V24.CREATE_SUBSCRIPTION_HISTORY(
...
prompt Compiling body for procs_taxes_cru.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_TAXES_CRU_V24" AS
...
prompt Compiling body for procs_transaction_cru.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_TRANSACTION_CRU_V24" AS
...
BEGIN
  -- Create history
  PROCS_HISTORY_V24.CREATE_TRANSACTION_HISTORY(
...
prompt Compiling body for procs_address.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_ADDRESS_V24" AS
...
BEGIN
  ...
  CORE_OWNER.PROCS_ADDRESS_CRU_V24.CREATE_ADDRESS(
...
BEGIN
  CORE_OWNER.PROCS_ADDRESS_CRU_V24.UPDATE_ADDRESS(
    in_address_id         => in_address_id,
    in_address1           => in_address1,
    in_address2           => in_address2,
    in_city               => in_city,
...
prompt Compiling body for procs_adjustments.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_ADJUSTMENTS_V24" AS
...
BEGIN
  ...
  IF in_is_credit != GLOBAL_CONSTANTS_V24.TRUE AND in_is_credit != GLOBAL_CONSTANTS_V24.FALSE THEN
...
BEGIN
  ...
  --create history
  PROCS_HISTORY_V24.CREATE_INVOICE_ADJ_HISTORY(
...
prompt Compiling body for procs_adx.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_ADX_V24" AS
...
prompt Compiling body for procs_amazon.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_AMAZON_V24" AS

PROCEDURE AASR_PURCHASE_TOKEN_USED(
  in_purchase_token IN AMAZON_APPSTORE_RECEIPT.PURCHASE_TOKEN%TYPE,
  out_data          OUT NUMBER
) AS
SPROC_NAME CONSTANT VARCHAR2(32) := 'AASR_PURCHASE_TOKEN_USED';
BEGIN
  SELECT
    COUNT(1)
  INTO OUT_DATA
  FROM AMAZON_APPSTORE_RECEIPT
  WHERE PURCHASE_TOKEN = in_purchase_token;
  EXCEPTION
  WHEN NO_DATA_FOUND THEN
  out_data := 0;
END;
...
BEGIN
  CORE_OWNER.PROCS_AMAZON_CRU_V24.UPDATE_AMAZON_APPSTORE_RECEIPT(
    in_id              => in_id,
    in_updated_by      => in_updated_by,
    in_last_check_date => SYSDATE
  );
END UPDATE_AASR_LAST_CHECK_DATE;

PROCEDURE EXPIRED_AASR_SUB_IDS(
  out_data   OUT SYS_REFCURSOR
) AS
SPROC_NAME CONSTANT VARCHAR2(32) := 'EXPIRED_AASR_SUB_IDS';
BEGIN
  OPEN out_data FOR
  SELECT
    *
  FROM
    (
      SELECT
        AMAZON_APPSTORE_RECEIPT.SUBSCRIPTION_ID
      FROM AMAZON_APPSTORE_RECEIPT
        INNER JOIN SUBSCRIPTION ON SUBSCRIPTION.ID = AMAZON_APPSTORE_RECEIPT.SUBSCRIPTION_ID
      WHERE
        AMAZON_APPSTORE_RECEIPT.END_DATE <= SYSDATE AND
        SUBSCRIPTION.SUBSCRIPTION_STATUS_ID = GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE
      ORDER BY dbms_random.value
    )
  WHERE
    ROWNUM <= 1000;
END EXPIRED_AASR_SUB_IDS;
...
SPROC_NAME CONSTANT VARCHAR2(32) := 'UPDATE_AASR_PURCHASE_TOKEN';
BEGIN
    CORE_OWNER.PROCS_AMAZON_CRU_V24.UPDATE_AMAZON_APPSTORE_RECEIPT(
        in_id               => in_id,
        in_updated_by       => in_updated_by,
        in_purchase_token   => in_purchase_token
    );
END UPDATE_AASR_PURCHASE_TOKEN;
...
SPROC_NAME CONSTANT VARCHAR2(32) := 'UPDATE_AASR_END_DATE';
BEGIN
  CORE_OWNER.PROCS_AMAZON_CRU_V24.UPDATE_AMAZON_APPSTORE_RECEIPT(
      in_id         => in_id,
      in_updated_by => in_updated_by,
...
END UPDATE_AASR_END_DATE;

PROCEDURE AASR_RECEIPTS_WITH_ACTIVE_SUBS(
  out_data   OUT SYS_REFCURSOR,
  in_process_name IN PROCESS_RETRY_THROTTLE.PROCESS_NAME%TYPE
) AS
SPROC_NAME CONSTANT VARCHAR2(32) := 'AASR_RECEIPTS_WITH_ACTIVE_SUBS';
BEGIN
  OPEN out_data FOR
  SELECT
    *
  FROM (
    SELECT
      AMAZON_APPSTORE_RECEIPT.ID,
      AMAZON_APPSTORE_RECEIPT.SUBSCRIPTION_ID,
      AMAZON_APPSTORE_RECEIPT.USER_ID,
      AMAZON_APPSTORE_RECEIPT.ITEM_TYPE,
      AMAZON_APPSTORE_RECEIPT.START_DATE,
      AMAZON_APPSTORE_RECEIPT.END_DATE,
      AMAZON_APPSTORE_RECEIPT.SKU,
      AMAZON_APPSTORE_RECEIPT.PURCHASE_TOKEN,
      AMAZON_APPSTORE_RECEIPT.CREATE_DATE,
      AMAZON_APPSTORE_RECEIPT.CREATED_BY,
      AMAZON_APPSTORE_RECEIPT.UPDATE_DATE,
      AMAZON_APPSTORE_RECEIPT.UPDATED_BY,
      AMAZON_APPSTORE_RECEIPT.LAST_CHECK_DATE
    FROM AMAZON_APPSTORE_RECEIPT
      INNER JOIN SUBSCRIPTION ON SUBSCRIPTION.ID = AMAZON_APPSTORE_RECEIPT.SUBSCRIPTION_ID
    WHERE
      SUBSCRIPTION.SUBSCRIPTION_STATUS_ID = GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE
      ...
      (
          ...
          WHERE PROCESS_NAME = in_process_name
                AND GENERIC_ID = AMAZON_APPSTORE_RECEIPT.ID
      )
    ORDER BY dbms_random.value
  )
  WHERE
    ROWNUM <= 1000;
END AASR_RECEIPTS_WITH_ACTIVE_SUBS;
...
SPROC_NAME         CONSTANT VARCHAR2(32) := 'ADD_AMAZON_APPSTORE_RECEIPT';
BEGIN
  CORE_OWNER.PROCS_AMAZON_CRU_V24.ADD_AMAZON_APPSTORE_RECEIPT(
      in_subscription_id =>  in_subscription_id,
...
SPROC_NAME CONSTANT VARCHAR2(32) := 'AMAZON_APPSTORE_RECEIPTS';
BEGIN
  OPEN out_data FOR
  SELECT
    AMAZON_APPSTORE_RECEIPT.ID,
  ...
  FROM AMAZON_APPSTORE_RECEIPT
    INNER JOIN SUBSCRIPTION ON SUBSCRIPTION.ID = AMAZON_APPSTORE_RECEIPT.SUBSCRIPTION_ID
  WHERE AMAZON_APPSTORE_RECEIPT.USER_ID = in_user_id AND
        AMAZON_APPSTORE_RECEIPT.SKU = in_sku AND
        SUBSCRIPTION.SUBSCRIPTION_STATUS_ID = GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE;
END AMAZON_APPSTORE_RECEIPTS;
...
BEGIN
  ...
  CORE_OWNER.PROCS_AMAZON_CRU_V24.CREATE_AMAZON_SUB(
    out_id              =>  out_id,
    in_subscription_id  =>  in_subscription_id,
...
prompt Compiling body for procs_cupy.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_CUPY" AS
  ...
  BEGIN
    ...
    FROM CREDIT_CARD cc, CC_UPDATE u, CC_REQUEST_FILE rf
  ...
  BEGIN
    ...
    UPDATED_BY = in_updated_by,
    CREDIT_CARD_STATUS_ID = GLOBAL_STATUSES_V24.CREDIT_CARD_DISABLED
  ...
  BEGIN
    PROCS_ACCOUNT_V24.GET_GROUP_ID_BY_ACCOUNT_ID(
      in_account_id => in_account_id,
  ...
  BEGIN
    PROCS_LICENSE_V24.GET_LICENSE_BY_ID(
      in_license_id => in_license_id,
...
prompt Compiling body for procs_entitlement.sql
CREATE OR REPLACE
PACKAGE BODY PROCS_ENTITLEMENT_V24 AS
  ...
  GROUP BY
    P.ENTITLEMENT, 0, P.IS_SHAREABLE, P.NAME
UNION ALL
  SELECT
    P.ENTITLEMENT,
    P.NAME,
...
prompt Compiling body for procs_history.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_HISTORY_V24" AS
...
SPROC_NAME CONSTANT VARCHAR2(32) := 'CREATE_AASR_HISTORY';
var_id              AMAZON_APPSTORE_RECEIPT.ID%TYPE;
var_subscription_id AMAZON_APPSTORE_RECEIPT.SUBSCRIPTION_ID%TYPE;
var_user_id         AMAZON_APPSTORE_RECEIPT.USER_ID%TYPE;
var_item_type       AMAZON_APPSTORE_RECEIPT.ITEM_TYPE%TYPE;
var_start_date      AMAZON_APPSTORE_RECEIPT.START_DATE%TYPE;
var_end_date        AMAZON_APPSTORE_RECEIPT.END_DATE%TYPE;
var_sku             AMAZON_APPSTORE_RECEIPT.SKU%TYPE;
var_purchase_token  AMAZON_APPSTORE_RECEIPT.PURCHASE_TOKEN%TYPE;
var_create_date     AMAZON_APPSTORE_RECEIPT.CREATE_DATE%TYPE;
var_created_by      AMAZON_APPSTORE_RECEIPT.CREATED_BY%TYPE;
var_update_date     AMAZON_APPSTORE_RECEIPT.UPDATE_DATE%TYPE;
var_updated_by      AMAZON_APPSTORE_RECEIPT.UPDATED_BY%TYPE;
var_last_check_date AMAZON_APPSTORE_RECEIPT.LAST_CHECK_DATE%TYPE;
BEGIN
  SELECT
    AMAZON_APPSTORE_RECEIPT.ID,
    AMAZON_APPSTORE_RECEIPT.SUBSCRIPTION_ID,
  ...
  INTO
    ...
    var_last_check_date
  FROM
    AMAZON_APPSTORE_RECEIPT
  WHERE
    AMAZON_APPSTORE_RECEIPT.ID = in_id;

  CORE_HIST_OWNER.PUBLIC_PROCS_CORE_V24.CREATE_AASR_HISTORY(
      var_id,
      var_subscription_id,
      var_user_id,
      var_item_type,
...
BEGIN
  ...
  BEGIN
    CORE_HIST_OWNER.PUBLIC_PROCS_CORE_V24.CREATE_ADDRESS_HISTORY(
      in_address_id,
      in_system_activity_reason_id,
      var_address1,
      var_address2,
...
BEGIN
  ...
  BEGIN
    CORE_HIST_OWNER.PUBLIC_PROCS_CORE_V24.CREATE_ACCOUNT_HISTORY(
      in_account_id,
      var_suspend_date,
      var_group_id,
      var_updated_by,
...
BEGIN
  ...
  BEGIN
    CORE_HIST_OWNER.PUBLIC_PROCS_CORE_V24.CREATE_SUBSCRIPTION_HISTORY(
      in_subscription_id,
      var_account_id,
      var_purchase_date,
      var_offer_chain_id,
...
BEGIN
  ...
  BEGIN    
    CORE_HIST_OWNER.PUBLIC_PROCS_CORE_V24.CREATE_CREDIT_CARD_HISTORY(
      in_credit_card_id,
      var_account_id,
      var_instrument_name,
      var_private_card_holder_name,
...
BEGIN
  ...
  BEGIN
    CORE_HIST_OWNER.PUBLIC_PROCS_CORE_V24.CREATE_PAYPAL_HISTORY(
      in_paypal_id,
      var_account_id,
      var_instrument_name,
      var_private_email_address,
...
BEGIN
  ...
  BEGIN
    CORE_HIST_OWNER.PUBLIC_PROCS_CORE_V24.CREATE_GIFT_CERT_HISTORY(
...
BEGIN
  ...
  BEGIN
    CORE_HIST_OWNER.PUBLIC_PROCS_CORE_V24.CREATE_TRANSACTION_HISTORY(
      in_transaction_id,
      var_transaction_status_id,
      var_transaction_amount,
      var_updated_by,
...
BEGIN
  ...
  BEGIN
    CORE_HIST_OWNER.PUBLIC_PROCS_CORE_V24.CREATE_INVOICE_HISTORY(
      in_invoice_id,
      var_invoice_status_id,
      var_updated_by,
      var_update_date,
...
BEGIN
  ...
  BEGIN
    CORE_HIST_OWNER.PUBLIC_PROCS_CORE_V24.CREATE_LICENSE_HISTORY(
      in_license_id,
      var_license_status_id,
      var_needs_entitlements,
      var_start_date,
...
BEGIN
  ...
  BEGIN
    CORE_HIST_OWNER.PUBLIC_PROCS_CORE_V24.CREATE_CHARGE_HISTORY (
      in_charge_id,
      var_invoice_id,
      var_transaction_id,
      var_instrument_type_id,
...
BEGIN
  ...
  BEGIN
    CORE_HIST_OWNER.PUBLIC_PROCS_CORE_V24.CREATE_INVOICE_ADJ_HISTORY (
      var_invoice_adj_id,
      var_invoice_id,
      var_is_credit,
      var_charge_id,
...
prompt Compiling body for procs_itunes_receipt.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_ITUNES_RECEIPT_V24" AS
...
BEGIN
  ...
  CORE_OWNER.PROCS_ITUNES_RECEIPT_CRU_V24.CREATE_RECEIPT(
    out_id              =>  out_id,
    in_subscription_id  =>  in_subscription_id,
...
BEGIN
  ...
  CORE_OWNER.PROCS_ITUNES_RECEIPT_CRU_V24.UPDATE_RECEIPT(
    in_id => in_id,
    in_receipt => in_receipt,
    in_status => in_status,
    in_quantity => in_quantity,
...
BEGIN
	CORE_OWNER.PROCS_ITUNES_RECEIPT_CRU_V24.LINK_ITUNES_RECEIPT(
		in_id => in_id,
		in_subscription_id => in_subscription_id,
		in_updated_by => in_updated_by
		);
...
BEGIN
  CORE_OWNER.PROCS_ITUNES_RECEIPT_CRU_V24.MARK_RECEIPT_CHECKED(
    in_id => in_id
  );
END MARK_RECEIPT_CHECKED;
...
(
  ...
  (
    ...
    WHERE
      ...
      (S.subscription_status_id in (GLOBAL_STATUSES_V24.SUBSCRIPTION_SUSPENDED, GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE) or (S.subscription_status_id is null and IR.status != 21006)) AND
...
prompt Compiling body for procs_line_items.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_LINE_ITEMS_V24" AS
...
BEGIN
  ...
  LOOP
    ...
    var_line_item_price := PROCS_COMMON_V24.ROUND_10_6_TO_10_2(var_product_offering_price);
    ...
    PROCS_LINE_ITEMS_CRU_V24.CREATE_LINE_ITEM(
    ...
    LOOP
      PROCS_LINE_ITEMS_CRU_V24.CREATE_DISCOUNT_LINE_ITEM(
...
BEGIN
  ...
  -- Update line item
  PROCS_LINE_ITEMS_CRU_V24.UPDATE_LINE_ITEM(
...
BEGIN
  PROCS_LINE_ITEMS_V24.CALCULATE_LINE_ITEM_AMOUNT(in_line_item_id, var_invoice_amount);
...
prompt Compiling body for procs_locking.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_LOCKING_V24" AS
...
WHEN ACCOUNT_ALREADY_INITIALIZED THEN
  PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.STATE_ERROR,
...
prompt Compiling body for procs_offer_chain.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_OFFER_CHAIN_V24" AS

PROCEDURE IS_GC_OFFER_CHAIN(
  in_offer_chain_id IN OFFER_CHAIN.ID%TYPE,
  out_data OUT NUMBER
) AS
SPROC_NAME CONSTANT VARCHAR2(32) := 'IS_GC_OFFER_CHAIN';
var_purchase_oc NUMBER := 0;
var_redeem_oc   NUMBER := 0;
BEGIN
  SELECT
    COUNT(1) INTO var_purchase_oc
  FROM
    Offer_Chain
  WHERE
    Offer_Chain.Id                      = in_offer_chain_id
    AND Offer_Chain.Is_Gift_Certificate = GLOBAL_CONSTANTS_V24.TRUE;

  SELECT
    COUNT(1) INTO var_redeem_oc
  FROM
    Offer_Chain_Meta_Data
  WHERE
    Offer_Chain_Meta_Data.Name = 'redemption offer chain id'
    AND REGEXP_LIKE(Offer_Chain_Meta_Data.Value,'(^|\s|,)' || TO_CHAR(in_offer_chain_id) || '(,|\s|$)');

  out_data := var_purchase_oc + var_redeem_oc;
END;
...
PROCEDURE OC_ID_BY_ITUNES_PRODUCT_ID(
  ...
  SPROC_NAME CONSTANT VARCHAR2(32) := 'OC_ID_BY_ITUNES_PRODUCT_ID';
BEGIN
  SELECT
    ID INTO out_data
  FROM
    OFFER_CHAIN
  WHERE
    ITUNES_PRODUCT_ID = in_itunes_product_id;
EXCEPTION
  WHEN NO_DATA_FOUND THEN NULL;
END OC_ID_BY_ITUNES_PRODUCT_ID;

PROCEDURE OFFER_CHAIN_ID_BY_AMAZON_SKU(
  in_amazon_appstore_sku IN  OFFER_CHAIN.AMAZON_APPSTORE_SKU%TYPE,
  out_data               OUT OFFER_CHAIN.ID%TYPE
) AS
SPROC_NAME CONSTANT VARCHAR2(32) := 'OFFER_CHAIN_ID_BY_AMAZON_SKU';
BEGIN
  SELECT
    ID INTO out_data
  FROM
    OFFER_CHAIN
  WHERE
    AMAZON_APPSTORE_SKU = in_amazon_appstore_sku;
EXCEPTION
  WHEN NO_DATA_FOUND THEN NULL;
END OFFER_CHAIN_ID_BY_AMAZON_SKU;

PROCEDURE GET_OFFER_CHAIN_BY_ID (
  in_offer_chain_id IN   NUMBER,
  out_result_set    OUT  SYS_REFCURSOR
) AS
SPROC_NAME CONSTANT VARCHAR2(32) := 'GET_OFFER_CHAIN_BY_ID';
BEGIN
  ...
  OPEN out_result_set FOR
    SELECT
      ...
      OC.AMAZON_APPSTORE_SKU,
      OC.ITUNES_PRODUCT_ID
    FROM 
      OFFER_CHAIN OC
    WHERE
...
BEGIN
  ...
  SELECT
    ...
    PROCS_OFFER_CHAIN_V24.CALCULATE_OFFER_CHAIN_AMOUNT(OCH.ID) AS PRICE,
    PROCS_OFFER_CHAIN_V24.IS_OFFER_CHAIN_CANCELABLE(OCH.ID) AS IS_CANCELABLE,
...
BEGIN
  ...
  SELECT
    ...
    PROCS_OFFER_CHAIN_V24.CALCULATE_OFFER_AMOUNT(OOCH.OFFER_ID) AS PRICE
  FROM
    OFFER O
...
BEGIN
  ...
  SELECT
    ...
    OFFER_CHAIN.OFFER_CHAIN_STATUS_ID,
    OFFER_CHAIN.IS_GIFT_CERTIFICATE,
    PROCS_OFFER_CHAIN_V24.IS_OFFER_CHAIN_CANCELABLE(OFFER_CHAIN.ID) AS "IS_CANCELABLE",
  ...
  WHERE
    ...
    and OFFER_CHAIN.OFFER_CHAIN_STATUS_ID = GLOBAL_STATUSES_V24.OFFER_CHAIN_ACTIVE;
...
BEGIN
  ...
  WHERE
    OFFER_OFFER_CHAIN.OFFER_CHAIN_ID = in_offer_chain_id
    AND OFFER_OFFER_CHAIN.NUM_RECURRENCES = GLOBAL_ENUMS_V24.OFFER_REC_INFINITY;
  ...
  LOOP
    ...
    PROCS_COMMON_V24.ISO8601DURATION_TO_INTERVALS(var_offer_duration, var_offer_years, var_offer_months, var_offer_days);
...
BEGIN
  ...
  BEGIN
    PROCS_OFFER_CHAIN_V24.GET_FIRST_OFFER(
...
BEGIN
  ...
  IF temp_count > 0 THEN
    RETURN GLOBAL_CONSTANTS_V24.TRUE;
  ELSE
    RETURN GLOBAL_CONSTANTS_V24.FALSE;
  END IF;
...
BEGIN
  ...
  BEGIN
    PROCS_COMMON_V24.ISO8601DURATION_TO_INTERVALS(
      var_offer_duration,
      out_years,
      out_months,
      out_days
...
BEGIN
  ...
  var_offer_end_date := ( ( in_start_date
    + GLOBAL_CONSTANTS_V24.ONE_DAY_INTERVAL * var_offer_days )
    + GLOBAL_CONSTANTS_V24.ONE_MONTH_INTERVAL * var_offer_months )
    + GLOBAL_CONSTANTS_V24.ONE_YEAR_INTERVAL * var_offer_years;
...
var_product_eligibility_limit NUMBER := 1;
-- EXCEPTIONS
...
BEGIN
  ...
  PROCS_OFFER_CHAIN_V24.GET_OFF_CHAINS_SAME_PRODUCTS(
  ...
  LOOP
    ...
    IF in_use_eligibility_rules = GLOBAL_CONSTANTS_V24.FALSE THEN
...
BEGIN
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        var_is_cancelable := GLOBAL_CONSTANTS_V24.FALSE;
  END;

  IF var_is_cancelable = GLOBAL_CONSTANTS_V24.FALSE THEN
    RETURN GLOBAL_CONSTANTS_V24.FALSE;
  END IF;
...
BEGIN
  ...
  BEGIN
    ...
    WHERE
      NAME LIKE GLOBAL_CONSTANTS_V24.MAX_CONCURRENT_SUBS;

    IF var_max_concurrent_subs_str = GLOBAL_CONSTANTS_V24.MAX_CONCURRENT_SUBS_UNLIM THEN
      var_max_concurrent_subs := GLOBAL_CONSTANTS_V24.INFINITY;
    ELSE
  ...
  FROM
    (
      ...
      FROM
        ...
        INNER JOIN PRODUCT_OFFERING ON OFFER_PRODUCT_OFFERING.PRODUCT_OFFERING_ID = PRODUCT_OFFERING.ID
		INNER JOIN PRODUCT ON PRODUCT.ID = PRODUCT_OFFERING.PRODUCT_ID
	  where PRODUCT.ENTITLEMENT <> 'AAA'
      GROUP BY
        PRODUCT_OFFERING.PRODUCT_ID
    )
    INNER JOIN
...
BEGIN
  ...
  SELECT
    ...
    och.DESCRIPTION,
    och.OFFER_CHAIN_STATUS_ID,
    PROCS_OFFER_CHAIN_V24.CALCULATE_OFFER_CHAIN_AMOUNT(och.id) as amount,
    och.ADOPTABILITY_WINDOW_START_DATE,
    och.ADOPTABILITY_WINDOW_END_DATE,
    PROCS_OFFER_CHAIN_V24.IS_OFFER_CHAIN_CANCELABLE(och.id) as is_cancelable,
    och.IS_GIFT_CERTIFICATE,
  ...
  WHERE
    ...
    and och.OFFER_CHAIN_STATUS_ID = GLOBAL_STATUSES_V24.OFFER_CHAIN_ACTIVE
    and och.id in (
      SELECT DISTINCT
        och2.id
      from
...
BEGIN
  ...
  var_is_eligible := GLOBAL_CONSTANTS_V24.TRUE;
  ...
  IF (var_is_gc IS NULL OR var_is_gc != 1) THEN
    ...
    FOR f_offer_chain IN (
        ...
        WHERE
          ...
          AND (S.SUBSCRIPTION_STATUS_ID = GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE
               OR S.SUBSCRIPTION_STATUS_ID = GLOBAL_STATUSES_V24.SUBSCRIPTION_IN_GRACE_PERIOD)
    )
    LOOP
      BEGIN
        IF (
          PROCS_OFFER_CHAIN_V24.CHECK_FOR_SAME_PRODUCTS(
            in_offer_chain_id,
            f_offer_chain.offer_chain_id,
            GLOBAL_CONSTANTS_V24.TRUE
          ) = GLOBAL_CONSTANTS_V24.TRUE
        ) THEN
          var_is_eligible := GLOBAL_CONSTANTS_V24.FALSE;
...
prompt Compiling body for procs_polling_sync.sql
...
prompt Compiling body for procs_product.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_PRODUCT_V24" AS
...
prompt Compiling body for procs_reporting.sql
CREATE OR REPLACE
...
----
    ...
    begin
        open out_cursor for
            ...
            select p.id product_id, p.name, p.entitlement, p.is_shareable, p.tax_category_id, p.product_status_id, ps.value product_status
                  ,p.create_date
            from product p
                ,product_status ps
                , ids
...
prompt Compiling body for procs_reports.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_REPORTS_V24" AS
...
BEGIN
  ...
  WHERE
    ...
    AND PROCS_CHARGE_V24.IS_CHARGE_COLLECTED(CHARGE.ID) = 1 -- GLOBAL_CONSTANTS_V24.TRUE
...
prompt Compiling body for procs_system.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_SYSTEM_V24" AS
...
prompt Compiling body for procs_taxes.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_TAXES_V24" AS
...
BEGIN
  ...
  PROCS_TAXES_CRU_V24.CREATE_TAX(
    var_new_tax_id,
    in_tax_type_id,
    in_calculated_amount,
    in_created_by,
...
BEGIN
  ...
  IF var_countries_count > 1 THEN
    ...
    var_result := GLOBAL_CONSTANTS_V24.TRUE;
  ELSIF var_countries_count = 1 THEN
    var_result := GLOBAL_CONSTANTS_V24.TRUE;
  ELSE
    var_result := GLOBAL_CONSTANTS_V24.FALSE;
  END IF;
...
prompt Compiling body for procs_transaction.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_TRANSACTION_V24" AS
...
BEGIN
  ...
  PROCS_TRANSACTION_CRU_V24.CREATE_TRANSACTION(
...
BEGIN
  ...
  IF in_trans_attempt_status != GLOBAL_STATUSES_V24.TRANS_ATTEMPT_IN_PROGRESS
    AND in_trans_attempt_status != GLOBAL_STATUSES_V24.TRANS_ATTEMPT_SUCCESS
    AND in_trans_attempt_status != GLOBAL_STATUSES_V24.TRANS_ATTEMPT_FAILED THEN
    RAISE BAD_TRANS_ATTEMPT_STATUS;
  END IF;
  
  var_transaction_attempt_id := NULL;
  PROCS_TRANSACTION_CRU_V24.CREATE_TRANSACTION_ATTEMPT(
  ...
  PROCS_TRANSACTION_CRU_V24.UPDATE_TRANSACTION(
    in_transaction_id => in_transaction_id,
...
BEGIN
  ...
  -- Check that transaction status is correct
  IF    in_transaction_status_id != GLOBAL_STATUSES_V24.TRANSACTION_PENDING
    AND in_transaction_status_id != GLOBAL_STATUSES_V24.TRANSACTION_CLOSED
    AND in_transaction_status_id != GLOBAL_STATUSES_V24.TRANSACTION_CHARGEBACK
    AND in_transaction_status_id != GLOBAL_STATUSES_V24.TRANSACTION_DECLINED THEN
  ...
  IF  in_transaction_status_id = GLOBAL_STATUSES_V24.TRANSACTION_CHARGEBACK  OR in_transaction_status_id = GLOBAL_STATUSES_V24.TRANSACTION_DECLINED
  THEN
    PROCS_CHARGE_V24.UPDATE_CHARGE_STATUS(temp_charge_id,GLOBAL_STATUSES_V24.CHARGEBACK_SETTLED,in_updated_by);
  END IF;
...
BEGIN
  ...
  IF  in_is_settled = 1
  THEN
    PROCS_CHARGE_V24.UPDATE_CHARGE_STATUS(temp_charge_id,GLOBAL_STATUSES_V24.CHARGE_SETTLED,in_updated_by);
  END IF;
...
BEGIN
  ...
  IF in_transaction_attempt_status != GLOBAL_STATUSES_V24.TRANS_ATTEMPT_IN_PROGRESS
    AND in_transaction_attempt_status != GLOBAL_STATUSES_V24.TRANS_ATTEMPT_SUCCESS
    AND in_transaction_attempt_status != GLOBAL_STATUSES_V24.TRANS_ATTEMPT_FAILED THEN
    RAISE BAD_TRANS_ATTEMPT_STATUS;
  END IF;

  PROCS_TRANSACTION_CRU_V24.UPDATE_TRANSACTION_ATTEMPT(
...
BEGIN
  ...
  WHERE
    TRANSACTION_ATTEMPT.TRANSACTION_ID = in_transaction_id
    AND TRANSACTION_ATTEMPT.TRANSACTION_ATTEMPT_STATUS_ID = GLOBAL_STATUSES_V24.TRANS_ATTEMPT_FAILED;
...
BEGIN
  ...
  WHERE
    TRANSACTION_ATTEMPT.TRANSACTION_ID = in_transaction_id
    AND TRANSACTION_ATTEMPT.TRANSACTION_ATTEMPT_STATUS_ID = GLOBAL_STATUSES_V24.TRANS_ATTEMPT_SUCCESS;
...
BEGIN
  ...
  WHERE
    INVOICE.ID = in_invoice_id
    AND TRANSACTION.IS_REFUND = GLOBAL_CONSTANTS_V24.TRUE
    AND TRANSACTION.TRANSACTION_AMOUNT <= 0
    AND TRANSACTION.TRANSACTION_STATUS_ID = GLOBAL_STATUSES_V24.TRANSACTION_CLOSED;
...
BEGIN
  ...
  PROCS_TRANSACTION_CRU_V24.READ_TRANSACTION(
    in_transaction_id => in_transaction_id,
    out_result_set    => out_result_set
  );
...
BEGIN
  ...
  BEGIN
    PROCS_TRANSACTION_CRU_V24.UPDATE_TRANSACTION_ORDER_ID(
      in_transaction_id => in_transaction_id,
      in_order_id       => in_order_id,
      in_updated_by     => in_updated_by
    );
    EXCEPTION
      WHEN OTHERS THEN
        IF SQLCODE = APP_EXCEPTION_CODES_V24.NOT_FOUND THEN
...
BEGIN
  ...
  WHERE
    ...
    AND T.TAX_TYPE_ID IN (
      SELECT GLOBAL_ENUMS_V24.TAX_TYPE_VAT FROM DUAL
    );
...
BEGIN
  ...
  WHERE
    TRANSACTION_ATTEMPT.TRANSACTION_ID = in_transaction_id
    AND TRANSACTION_ATTEMPT.TRANSACTION_ATTEMPT_STATUS_ID IN ( SELECT GLOBAL_STATUSES_V24.TRANS_ATTEMPT_SUCCESS FROM DUAL );
...
prompt Compiling body for procs_license.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_LICENSE_V24" AS
...
BEGIN
  ...
  PROCS_LICENSE_CRU_V24.CREATE_LICENSE(
...
BEGIN
  ...
  IF in_license_status != GLOBAL_STATUSES_V24.LICENSE_CLOSED
     AND in_license_status != GLOBAL_STATUSES_V24.LICENSE_ACTIVE
     AND in_license_status != GLOBAL_STATUSES_V24.LICENSE_IN_GRACE_PERIOD THEN
    RAISE BAD_LICENSE_STATUS;
  END IF;

  IF (in_ent_end is not null and in_ent_end = GLOBAL_CONSTANTS_V24.TRUE) then
    PROCS_LICENSE_CRU_V24.UPDATE_LICENSE(
      in_license_id        => in_license_id,
      in_updated_by        => in_updated_by,
  ...
  OPEN out_result_set FOR
    SELECT * FROM (
      SELECT
        ...
        INVOICE_ID,
        IS_EXTENSION,
        (SELECT VALUE FROM LICENSE_STATUS WHERE ID = GLOBAL_STATUSES_V22.LICENSE_ACTIVE) STATUS,
        NEEDS_ENTITLEMENTS,
        OFFER_ID,
        START_DATE,
        SUBSCRIPTION_ID,
      ...
      FROM
        LICENSE
      WHERE
        END_DATE <= SYSDATE + (in_hours_number / 24)
        AND LICENSE_STATUS_ID = GLOBAL_STATUSES_V22.LICENSE_ACTIVE
        AND NOT EXISTS (
          SELECT NULL
          FROM PROCESS_RETRY_THROTTLE
          WHERE PROCESS_NAME = SPROC_NAME
...
BEGIN
  ...
  BEGIN  
    var_next_offer_index := PROCS_OFFER_CHAIN_V24.GET_NEXT_OFFER_INDEX(
      var_offer_chain_id,
      var_license_current_index
    );
    EXCEPTION
  ...
  OPEN out_result_set FOR
    SELECT * FROM (
      ...
      FROM
        LICENSE
      WHERE
        NEEDS_ENTITLEMENTS = GLOBAL_CONSTANTS_V24.TRUE
      AND ROWNUM <= 5000
      ORDER BY dbms_random.value
) WHERE
  ROWNUM <= 500;
...
BEGIN
  ...
  IF in_needs_entitlements != GLOBAL_CONSTANTS_V24.TRUE
    AND in_needs_entitlements != GLOBAL_CONSTANTS_V24.FALSE THEN
...
BEGIN
  ...
  PROCS_LICENSE_CRU_V24.UPDATE_LICENSE(
    ...
    in_updated_by        => in_updated_by,
    in_needs_entitlements => GLOBAL_CONSTANTS_V24.TRUE,
    in_end_date          => in_end_date,
  ...
  FROM
      (
          ...
          ON
              l.invoice_id = i.id
          WHERE
              i.invoice_status_id = GLOBAL_STATUSES_V24.INVOICE_OPEN
          AND l.license_status_id = GLOBAL_STATUSES_V24.LICENSE_IN_GRACE_PERIOD
          ...
          AND NOT EXISTS
              (
                  ...
                  WHERE
                      c.invoice_id = i.id
                  AND c.charge_status_id = GLOBAL_STATUSES_V24.CHARGE_OPENED)
...
prompt Compiling body for procs_charge.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_CHARGE_V24" AS
...
BEGIN
  ...
  IF in_instrument_type_id != GLOBAL_ENUMS_V24.INSTRUMENT_CREDIT_CARD
    AND in_instrument_type_id != GLOBAL_ENUMS_V24.INSTRUMENT_PAYPAL THEN
  ...
  IF in_instrument_type_id = GLOBAL_ENUMS_V24.INSTRUMENT_CREDIT_CARD THEN
    IF PROCS_FIN_INSTRUMENTS_V24.IS_CREDIT_CARD_EXISTS(in_instrument_id) = GLOBAL_CONSTANTS_V24.FALSE THEN
      RAISE BAD_CREDIT_CARD_ID;
    END IF;
  ELSIF in_instrument_type_id = GLOBAL_ENUMS_V24.INSTRUMENT_PAYPAL THEN
    IF PROCS_FIN_INSTRUMENTS_V24.IS_PAYPAL_EXISTS(in_instrument_id) = GLOBAL_CONSTANTS_V24.FALSE THEN
      RAISE BAD_PAYPAL_ID;
    END IF;
  END IF;

  -- Create new charge
  PROCS_CHARGE_CRU_V24.CREATE_CHARGE(
...
(
  ...
  FROM
    ...
    INNER JOIN TRANSACTION ON CHARGE.TRANSACTION_ID = TRANSACTION.ID
  WHERE
    TRANSACTION.TRANSACTION_STATUS_ID = GLOBAL_STATUSES_V24.TRANSACTION_PENDING
    AND TRANSACTION.IS_REFUND = GLOBAL_CONSTANTS_V24.TRUE
    AND TRANSACTION.TRANSACTION_AMOUNT < 0
    AND NOT EXISTS
    (
...
BEGIN
  ...
  WHERE
    ...
    AND CHARGE.CHARGE_STATUS_ID = GLOBAL_STATUSES_V24.CHARGE_OPENED
    AND 
      TRANSACTION.TRANSACTION_STATUS_ID = GLOBAL_STATUSES_V24.TRANSACTION_PENDING;
...
BEGIN
  ...
  PROCS_CHARGE_CRU_V24.UPDATE_CHARGE(
    in_charge_id        => in_charge_id,
    in_charge_status_id => in_charge_status_id,
    in_updated_by       => in_updated_by
  );
...
BEGIN
  ...
  BEGIN
    is_transaction_collected := PROCS_TRANSACTION_V24.IS_TRANSACTION_COLLECTED(var_transaction_id);
    EXCEPTION
      WHEN OTHERS THEN
        EXCEPTION_MESSAGE := SQLERRM;
...
BEGIN
  ...
  WHERE
    CHARGE.INVOICE_ID = in_invoice_id
    AND CHARGE.CHARGE_STATUS_ID IN (GLOBAL_STATUSES_V24.CHARGE_PROCESSED, GLOBAL_STATUSES_V24.CHARGE_CAPTURED,
        GLOBAL_STATUSES_V24.CHARGE_SETTLED,GLOBAL_STATUSES_V24.CHARGE_CHARGEBACK_CLAIM, GLOBAL_STATUSES_V24.CHARGEBACK_SETTLED)
    AND PROCS_CHARGE_V24.IS_CHARGE_COLLECTED(CHARGE.ID) = GLOBAL_CONSTANTS_V24.TRUE;
...
prompt Compiling body for procs_subscription.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_SUBSCRIPTION_V24" AS
...
BEGIN
  ...
  PROCS_LICENSE_CRU_V24.UPDATE_LICENSE(
      ...
      in_license_status_id    => GLOBAL_STATUSES_V24.LICENSE_IN_GRACE_PERIOD
  );

  PROCS_SUBSCRIPTION_CRU_V24.UPDATE_SUBSCRIPTION(
      in_subscription_id        => var_subs_id,
      in_updated_by             => in_updater,
      in_subscription_status_id => GLOBAL_STATUSES_V24.SUBSCRIPTION_IN_GRACE_PERIOD
    );
...
BEGIN
  ...
  PROCS_LICENSE_CRU_V24.UPDATE_LICENSE(
      ...
      in_license_status_id    => GLOBAL_STATUSES_V24.LICENSE_ACTIVE
  );

  PROCS_SUBSCRIPTION_CRU_V24.UPDATE_SUBSCRIPTION(
      in_subscription_id        => var_subs_id,
      in_updated_by             => in_updater,
      in_subscription_status_id => GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE
  );
...
BEGIN
  ...
  IF var_account_status = GLOBAL_STATUSES_V24.ACCOUNT_DISABLED THEN
  ...
  IF var_offer_chain_status_id != GLOBAL_STATUSES_V24.OFFER_CHAIN_ACTIVE THEN
    RAISE BAD_OFFER_CHAIN_STATUS;
  END IF;

  -- Can not subscribe to Offer Chain for a Gift Certfiicate
  IF var_is_gift_certificate = GLOBAL_CONSTANTS_V24.TRUE THEN
  ...
  WHERE
    OFFER_CHAIN_ELIGIBILITY.OFFER_CHAIN_ID = in_offer_chain_id
    AND OFFER_CHAIN_ELIGIBILITY.NAME = GLOBAL_CONSTANTS_V24.GIFT_CERTIFICATE_REQUIRED
    AND OFFER_CHAIN_ELIGIBILITY.VALUE = GLOBAL_CONSTANTS_V24.ELIGIBILITY_FLAG_SET;
  ...
  WHERE
    ...
    AND (SUBSCRIPTION.SUBSCRIPTION_STATUS_ID = GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE
         OR SUBSCRIPTION.SUBSCRIPTION_STATUS_ID = GLOBAL_STATUSES_V24.SUBSCRIPTION_IN_GRACE_PERIOD);
  ...
  -- then deny purchase
  var_max_concurrent_subscrs := PROCS_OFFER_CHAIN_V24.GET_OFFER_CHAIN_MAX_CONC_SUBSC(in_offer_chain_id);
  IF var_max_concurrent_subscrs != GLOBAL_CONSTANTS_V24.INFINITY
  ...
  -- then deny purchase
  IF (in_check_dupe_products != GLOBAL_CONSTANTS_V24.FALSE) THEN
    FOR f_account_offer_chains IN (
      ...
      WHERE
        ...
        AND (
          SUBSCRIPTION_STATUS_ID = GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE
          OR SUBSCRIPTION_STATUS_ID = GLOBAL_STATUSES_V24.SUBSCRIPTION_SUSPENDED
          OR SUBSCRIPTION_STATUS_ID = GLOBAL_STATUSES_V24.SUBSCRIPTION_IN_GRACE_PERIOD)
    )
    LOOP
      IF PROCS_OFFER_CHAIN_V24.CHECK_FOR_SAME_PRODUCTS(
        in_offer_chain_1         => in_offer_chain_id,
        in_offer_chain_2         => f_account_offer_chains.OFFER_CHAIN_ID,
        in_use_eligibility_rules => GLOBAL_CONSTANTS_V24.TRUE
      ) = GLOBAL_CONSTANTS_V24.TRUE THEN
        ...
        FROM (
          ...
          WHERE
            ...
            AND (
              SUBSCRIPTION.SUBSCRIPTION_STATUS_ID = GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE
              OR SUBSCRIPTION.SUBSCRIPTION_STATUS_ID = GLOBAL_STATUSES_V24.SUBSCRIPTION_SUSPENDED
              OR SUBSCRIPTION.SUBSCRIPTION_STATUS_ID = GLOBAL_STATUSES_V24.SUBSCRIPTION_IN_GRACE_PERIOD)
          ORDER BY
            ID
        )
  ...
  BEGIN
    PROCS_OFFER_CHAIN_V24.GET_FIRST_OFFER(in_offer_chain_id, var_first_offer_id);
    EXCEPTION
      WHEN OTHERS THEN
        EXCEPTION_MESSAGE := SQLERRM;
  ...
  BEGIN
    PROCS_INVOICE_V24.CREATE_INVOICE(
      in_invoice_status => GLOBAL_STATUSES_V24.INVOICE_OPEN,
      in_created_by     => in_created_by,
  ...
  BEGIN
    PROCS_LINE_ITEMS_V24.ADD_LINE_ITEMS(
  ...
  PROCS_SUBSCRIPTION_CRU_V24.CREATE_SUBSCRIPTION(
    out_subscription_id       => var_new_subscription_id,
  ...
  BEGIN
    PROCS_LICENSE_V24.CREATE_LICENSE(
      in_status_id                => GLOBAL_STATUSES_V24.LICENSE_ACTIVE,
      in_needs_entitlements       => GLOBAL_CONSTANTS_V24.TRUE,
      ...
      in_invoice_id               => var_new_invoice_id,
      in_created_by               => in_created_by,
      in_is_extension             => GLOBAL_CONSTANTS_V24.FALSE,
      in_current_offer_index      => PROCS_OFFER_CHAIN_V24.GET_FIRST_OFFER_INDEX(in_offer_chain_id),
...
WHEN LIMIT_REACHED THEN
  PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.CONFLICT_ERROR,
...
BEGIN
  ...
  BEGIN
    PROCS_INVOICE_V24.CALCULATE_INVOICE_AMOUNT(
      in_invoice_id => in_invoice_id,
  ...
  IF in_instrument_type_id = GLOBAL_ENUMS_V24.INSTRUMENT_ZCI_INSTRUMENT
  ...
  IF var_invoice_amount = 0 THEN
    ...
    PROCS_INVOICE_CRU_V24.UPDATE_INVOICE(
      in_invoice_id                  => in_invoice_id,
      in_updated_by                  => in_created_by,
      in_invoice_status_id           => GLOBAL_STATUSES_V24.INVOICE_CLOSED
    );
  ELSE
    -- Create transaction and charge
    BEGIN
      PROCS_TRANSACTION_V24.CREATE_TRANSACTION(
        in_transaction_id         => NULL,
        in_status_id              => GLOBAL_STATUSES_V24.TRANSACTION_PENDING,
    ...
    BEGIN
      PROCS_CHARGE_V24.CREATE_CHARGE(
        in_invoice_id         => in_invoice_id,
        ...
        in_created_by         => in_created_by,
        in_charge_status_id   => GLOBAL_STATUSES_V24.CHARGE_OPENED,
        out_charge_id         => var_new_charge_id
      );
      EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
  ...
  IF var_subscription_status_id != GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE THEN
  ...
  BEGIN
    ...
    WHERE
      LICENSE.SUBSCRIPTION_ID = in_subs_id
        AND LICENSE.LICENSE_STATUS_ID = GLOBAL_STATUSES_V24.LICENSE_ACTIVE;
    EXCEPTION
    WHEN NO_DATA_FOUND THEN
  ...
  PROCS_LICENSE_CRU_V24.UPDATE_LICENSE(
    ...
    in_updated_by        => in_updated_by,
    in_license_status_id => GLOBAL_STATUSES_V24.LICENSE_CLOSED
  );
  ...
  PROCS_SUBSCRIPTION_CRU_V24.UPDATE_SUBSCRIPTION(
    ...
    in_subscription_status_id    => GLOBAL_STATUSES_V24.SUBSCRIPTION_SUSPENDED,
...
BEGIN
  ...
  SELECT
    ...
    PROCS_SUBSCRIPTION_V24.CALC_SUBSCRIPTION_END_DATE(SUBSCRIPTION.ID) as "END_DATE",
    PROCS_SUBSCRIPTION_V24.GET_RECENT_CHARGE(SUBSCRIPTION.ID) AS "RECENT_CHARGE",
    PROCS_SUBSCRIPTION_V24.GET_RENEWAL_DATE(SUBSCRIPTION.ID) AS "RENEWAL_DATE",
    PROCS_SUBSCRIPTION_V24.GET_BILLING_CYCLE(SUBSCRIPTION.ID) AS "BILLING_CYCLE",
    (
      SELECT
        ACCOUNT.GROUP_ID
        FROM ACCOUNT
    ...
    as "ENT_START_DATE",
    PROCS_SUBSCRIPTION_V24.IS_SUBSCRIPTION_CANCELABLE(SUBSCRIPTION.ID) AS "IS_CANCELABLE",
    ...
    PROCS_GROUP_ACCOUNT_V24.F_GET_NUM_OCCUPIED_GROUP_SEATS(GROUP_ACCOUNT.ID) GA_SEATS_USED,
    GROUP_ACCOUNT.IP GA_IP,
    PROCS_SUBSCRIPTION_V24.GET_GIFT_CERT_CODE_BY_SUB_ID(SUBSCRIPTION.ID) gift_certificate_code,
    PROCS_ACCOUNT_V24.GET_GRACE_START_DATE(SUBSCRIPTION.ID) GRACE_START_DATE,
    PROCS_ACCOUNT_V24.GET_GRACE_END_DATE(SUBSCRIPTION.ID) GRACE_END_DATE
  FROM
    SUBSCRIPTION
...
BEGIN
  ...
  PROCS_SUBSCRIPTION_CRU_V24.CREATE_SUBSCRIPTION_NOTE(
...
BEGIN
  ...
  IF var_subscription_status != GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE THEN
    -- TODO: Is suspended subscription has renewal date? (For the phase II)
    -- AND var_subscription_status != GLOBAL_STATUSES_V24.SUBSCRIPTION_SUSPENDED THEN
    RETURN NULL;
  END IF;
    ...
    FROM
      (
        ...
        FROM
          LICENSE
        WHERE
          LICENSE.LICENSE_STATUS_ID = GLOBAL_STATUSES_V24.LICENSE_ACTIVE
  ...
  IF PROCS_OFFER_CHAIN_V24.GET_NEXT_OFFER_INDEX(var_offer_chain_id, var_current_offer_index) IS NULL
...
BEGIN
  ...
  BEGIN
    ...
    WHERE
      -- TODO: Review
      -- LICENSE.LICENSE_STATUS_ID = GLOBAL_STATUSES_V24.LICENSE_PROCESSED
      -- AND
      SUBSCRIPTION.ID = in_subscription_id
      AND CHARGE.CHARGE_AMOUNT >= 0
      AND ROWNUM <= 1
...
BEGIN
  ...
  var_current_date := PROCS_COMMON_V24.NORMALIZE_DATE(SYSDATE);
  ...
  BEGIN
    ...
    WHERE
      SUBSCRIPTION.ID = in_subscription_id
      AND PROCS_COMMON_V24.NORMALIZE_DATE(LICENSE.END_DATE) > var_current_date
      AND PROCS_COMMON_V24.NORMALIZE_DATE(LICENSE.START_DATE) <= var_current_date
      AND ROWNUM <= 1
    ORDER BY
      LICENSE.ID DESC;
    EXCEPTION
  ...
  BEGIN
    PROCS_INVOICE_V24.CREATE_INVOICE(
      in_invoice_status => GLOBAL_STATUSES_V24.INVOICE_CLOSED,
      in_created_by     => in_updated_by,
      in_tax_exempt_id  => var_account_tax_exempt_id,
  ...
  BEGIN
    PROCS_LICENSE_V24.CREATE_LICENSE (
      ...
      in_created_by               => in_updated_by,
      in_is_extension             => GLOBAL_CONSTANTS_V24.TRUE,
  ...
  IF PROCS_COMMON_V24.NORMALIZE_DATE(var_current_license_end_date) >
     PROCS_COMMON_V24.NORMALIZE_DATE(in_effective_start_date) THEN
    BEGIN
      PROCS_LICENSE_V24.CREATE_LICENSE (
        in_status_id                => GLOBAL_STATUSES_V24.LICENSE_ACTIVE,
        in_needs_entitlements       => GLOBAL_CONSTANTS_V24.TRUE,
...
BEGIN
  ...
  IF var_current_offer_rec_number > last_license_offer_recurr_num THEN
    BEGIN
      PROCS_OFFER_CHAIN_V24.GET_OFFER_LENGTH(
        last_license_offer_id,
        var_offer_years,
        var_offer_months,
        var_offer_days
...
BEGIN
  ...
  IF var_future_licenses_count > 0 THEN
    out_result := GLOBAL_CONSTANTS_V24.TRUE;
  ELSE
    out_result := GLOBAL_CONSTANTS_V24.FALSE;
  END IF;
...
BEGIN
  ...
  PROCS_SUBSCRIPTION_V24.UPDATE_SUBSCRIPTION_STATUS(
    in_subscription_id        => in_subscription_id,
    in_updated_by             => in_updated_by,
    in_subscription_status_id => GLOBAL_STATUSES_V24.SUBSCRIPTION_CLOSED
  );
...
BEGIN
  ...
  FROM
    LICENSE
  WHERE
    LICENSE.LICENSE_STATUS_ID in (GLOBAL_STATUSES_V24.LICENSE_ACTIVE, GLOBAL_STATUSES_V24.LICENSE_IN_GRACE_PERIOD)
    AND LICENSE.SUBSCRIPTION_ID = in_subscription_id;
...
BEGIN
  ...
  select decode(count(1), 0, GLOBAL_CONSTANTS_V24.FALSE, GLOBAL_CONSTANTS_V24.TRUE) into var_revoke
  ...
  where
    ...
    oc.revoke_entitlements = GLOBAL_CONSTANTS_V24.TRUE and
    rownum < 2;

  select
    decode(s.subscription_status_id, GLOBAL_STATUSES_V24.SUBSCRIPTION_IN_GRACE_PERIOD, 1, 0)
  into
  ...
  var_billed := PROCS_INVOICE_V24.IS_INVOICE_PAYING_STARTED(in_invoice_id);
  var_refund := GLOBAL_CONSTANTS_V24.FALSE;

  -- Check that transaction for given invoice not started
  -- if refund enabled calculate and apply chargeback
  IF (
      var_billed = GLOBAL_CONSTANTS_V24.TRUE
    )THEN
    if (in_refundable = GLOBAL_CONSTANTS_V24.TRUE) then
      -- If started then we need to calculate refund
      BEGIN
        PROCS_INVOICE_V24.CALCULATE_INVOICE_CHARGEBACK(
          in_invoice_id,
          var_now,
      ...
      IF var_chargeback_amount > 0 THEN
        BEGIN
          PROCS_INVOICE_V24.APPLY_REFUND(
            in_invoice_id,
            var_chargeback_amount,
            in_updated_by,
        ...
        END;
        var_refund := GLOBAL_CONSTANTS_V24.TRUE;
      END IF;
    end if;
  ELSE

    FOR f_transaction_to_close IN (
      SELECT DISTINCT CHARGE.TRANSACTION_ID AS "ID" FROM CHARGE WHERE CHARGE.INVOICE_ID = in_invoice_id and CHARGE.CHARGE_STATUS_ID = GLOBAL_STATUSES_V24.CHARGE_OPENED
    )
    LOOP
      PROCS_TRANSACTION_V24.UPDATE_TRANSACTION_STATUS(
        in_transaction_id        => f_transaction_to_close.ID,
        in_updated_by            => in_updated_by,
        in_transaction_status_id => GLOBAL_STATUSES_V24.TRANSACTION_CLOSED
      );
    END LOOP;
    -- Needs to close charges. No refund.
    FOR f_charge_to_close IN (
      SELECT CHARGE.ID FROM CHARGE WHERE CHARGE.INVOICE_ID = in_invoice_id and CHARGE.CHARGE_STATUS_ID = GLOBAL_STATUSES_V24.CHARGE_OPENED
    )
    LOOP
      PROCS_CHARGE_V24.UPDATE_CHARGE_STATUS(
        in_charge_id        => f_charge_to_close.ID,
        in_updated_by       => in_updated_by,
        in_charge_status_id => GLOBAL_STATUSES_V24.CHARGE_CANCELED
      );
    END LOOP;

    PROCS_INVOICE_V24.UPDATE_INVOICE_STATUS(
      in_invoice_id                  => in_invoice_id,
      in_updated_by                  => in_updated_by,
      in_invoice_status_id           => GLOBAL_STATUSES_V24.INVOICE_CLOSED
    );
  ...
  END IF;
  -- update licenses
  IF(var_revoke = GLOBAL_CONSTANTS_V24.TRUE OR var_chargeback_amount > 0 OR (var_subscription_in_grace = GLOBAL_CONSTANTS_V24.FALSE AND var_billed = GLOBAL_CONSTANTS_V24.FALSE)) THEN
    FOR f_license_to_cancel IN (
      SELECT LICENSE.ID FROM LICENSE WHERE LICENSE.INVOICE_ID = in_invoice_id AND LICENSE.LICENSE_STATUS_ID = GLOBAL_STATUSES_V24.LICENSE_ACTIVE
    )
    LOOP
      PROCS_LICENSE_CRU_V24.UPDATE_LICENSE(
  ...
  ELSE
    FOR f_license_to_cancel IN (
      SELECT LICENSE.ID FROM LICENSE WHERE LICENSE.INVOICE_ID = in_invoice_id AND LICENSE.LICENSE_STATUS_ID in (GLOBAL_STATUSES_V24.LICENSE_ACTIVE, GLOBAL_STATUSES_V24.LICENSE_IN_GRACE_PERIOD)
    )
    LOOP
      PROCS_LICENSE_CRU_V24.UPDATE_LICENSE(
        in_license_id         => f_license_to_cancel.ID,
...
BEGIN
  ...
  IF var_current_subscr_status != GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE AND
     var_current_subscr_status != GLOBAL_STATUSES_V24.SUBSCRIPTION_SUSPENDED AND
     var_current_subscr_status != GLOBAL_STATUSES_V24.SUBSCRIPTION_IN_GRACE_PERIOD THEN
    RAISE BAD_SUBSCRIPTION_STATUS;
  END IF;
  ...
  BEGIN
    PROCS_SUBSCRIPTION_CRU_V24.UPDATE_SUBSCRIPTION(
      in_subscription_id        => in_subscription_id,
      in_subscription_status_id => GLOBAL_STATUSES_V24.SUBSCRIPTION_CANCELED,
  ...
  BEGIN
    ...
    WHERE
      LICENSE.SUBSCRIPTION_ID = in_subscription_id
      AND LICENSE.LICENSE_STATUS_ID = GLOBAL_STATUSES_V24.LICENSE_IN_GRACE_PERIOD
      AND ROWNUM <= 1
    ORDER BY
      CREATE_DATE DESC;
  EXCEPTION
  ...
  IF var_license_to_disgrace IS NOT NULL THEN
    IF in_revoke_entitlements = GLOBAL_CONSTANTS_V22.TRUE THEN
      PROCS_LICENSE_CRU_V22.UPDATE_LICENSE(
        in_license_id           => var_license_to_disgrace,
        in_license_status_id    => GLOBAL_STATUSES_V22.LICENSE_CLOSED,
...
BEGIN
  ...
  -- Check that subscription is active
  IF var_current_subscr_status != GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE
     AND var_current_subscr_status != GLOBAL_STATUSES_V24.SUBSCRIPTION_SUSPENDED THEN
    RAISE BAD_SUBSCRIPTION_STATUS;
  END IF;
  ...
  BEGIN
    PROCS_SUBSCRIPTION_CRU_V24.UPDATE_SUBSCRIPTION(
      in_subscription_id        => in_subscription_id,
      in_subscription_status_id => GLOBAL_STATUSES_V24.SUBSCRIPTION_FALSE_START,
      in_cancelation_date       => in_cancelation_date,
      in_updated_by             => in_updated_by,
...
BEGIN
  ...
  BEGIN
    ...
    EXCEPTION
        ...
        WHEN NO_DATA_FOUND THEN
            SELECT
                PROCS_OFFER_CHAIN_V24.IS_OFFER_CHAIN_CANCELABLE(offer_id)
...
BEGIN
  ...
  WHERE
    OFFER_PRODUCT_OFFERING.OFFER_ID IN (
      ...
      FROM
        SUBSCRIPTION
        JOIN LICENSE ON LICENSE.SUBSCRIPTION_ID = SUBSCRIPTION.ID AND LICENSE.NEEDS_ENTITLEMENTS = GLOBAL_CONSTANTS_V24.TRUE
      WHERE
        SUBSCRIPTION.ID = in_subscription_id
    );
...
BEGIN
  ...
  SELECT DISTINCT
    ...
    p.TAX_CATEGORY_ID po_tax_category_id,
  ...
  WHERE
    ...
    and po.product_id = p.id
    and l.license_status_id = GLOBAL_STATUSES_V24.LICENSE_ACTIVE
...
BEGIN
  ...
  FOR f_invoice IN (
    ...
    WHERE
      LICENSE.SUBSCRIPTION_ID = in_subscription_id
      AND LICENSE.LICENSE_STATUS_ID IN ( SELECT GLOBAL_STATUSES_V24.LICENSE_ACTIVE FROM DUAL )
  )
  LOOP
    ...
    LOOP
      ...
      IF f_charge.CHARGE_AMOUNT < 0
         AND f_charge.CHARGE_STATUS_ID = GLOBAL_STATUSES_V24.CHARGE_OPENED THEN
        var_local_result := GLOBAL_CONSTANTS_V24.TRUE;
      END IF;
...
BEGIN
  ...
  WHERE
    ...
    c.charge_status_id = GLOBAL_STATUSES_V24.CHARGE_OPENED and
    exists (
      select null
      from transaction t
      where
...
BEGIN
  OPEN out_result_set FOR
    ...
    AND s.subscription_status_id in (GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE, GLOBAL_STATUSES_V24.SUBSCRIPTION_IN_GRACE_PERIOD);
  ...
  FROM
      (
          ...
          ON
              a.id = s.account_id
          WHERE
              l.license_status_id = GLOBAL_STATUSES_V24.LICENSE_IN_GRACE_PERIOD
...
BEGIN
  OPEN out_result_set FOR
    ...
    WHERE
      s.subscription_status_id in (GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE, GLOBAL_STATUSES_V24.SUBSCRIPTION_IN_GRACE_PERIOD);
...
prompt Compiling body for procs_invoice.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_INVOICE_V24" AS
...
BEGIN
  OPEN out_result_set FOR
    ...
    WHERE
      ...
      AND Invoice.Invoice_Status_Id = GLOBAL_STATUSES_V24.INVOICE_OPEN;
END GET_INVOICE_IDS;
...
BEGIN
  ...
  PROCS_INVOICE_CRU_V24.CREATE_INVOICE(
    out_invoice_id                 => var_new_invoice_id,
    in_created_by                  => in_created_by,
...
BEGIN
  ...
  LOOP
    BEGIN
      PROCS_LINE_ITEMS_V24.CALCULATE_LINE_ITEM_AMOUNT(
...
BEGIN
  ...
  -- get group id
  BEGIN
    PROCS_INVOICE_V24.GET_ACCOUNT_BY_INVOICE_ID(in_invoice_id, out_group_id);
    EXCEPTION
      WHEN OTHERS THEN
        EXCEPTION_MESSAGE := SQLERRM;
        RAISE CAN_NOT_FIND_ACCOUNT;
  ...
  WHERE
    CHARGE.INVOICE_ID = in_invoice_id
    AND CHARGE.INSTRUMENT_TYPE_ID = GLOBAL_ENUMS_V24.INSTRUMENT_PAYPAL;
  ...
  WHERE
    CHARGE.INVOICE_ID = in_invoice_id
    AND CHARGE.INSTRUMENT_TYPE_ID = GLOBAL_ENUMS_V24.INSTRUMENT_CREDIT_CARD;
  ...
  WHERE
    CHARGE.INVOICE_ID = in_invoice_id
    AND CHARGE.INSTRUMENT_TYPE_ID = GLOBAL_ENUMS_V24.INSTRUMENT_GIFT_CERTIFICATE;
...
BEGIN
  ...
  IF in_invoice_status_id != GLOBAL_STATUSES_V24.INVOICE_OPEN
    AND in_invoice_status_id != GLOBAL_STATUSES_V24.INVOICE_CLOSED THEN
...
BEGIN
  ...
  FROM
    ...
    CHARGE c
  WHERE
    ta.TRANSACTION_ATTEMPT_STATUS_ID = GLOBAL_STATUSES_V24.TRANS_ATTEMPT_SUCCESS and
    ta.transaction_id = t.id and
    c.transaction_id = t.id and
    t.is_refund = GLOBAL_CONSTANTS_V24.FALSE and
  ...
  WHERE
    c.transaction_id = t.id and
    t.is_refund = GLOBAL_CONSTANTS_V24.FALSE and
    t.is_settled = GLOBAL_CONSTANTS_V24.TRUE and
    c.invoice_id = in_invoice_id
  ;
...
BEGIN
  -- Just a wrapper
  out_is_started := PROCS_INVOICE_V24.IS_INVOICE_PAYING_STARTED(in_invoice_id);
...
BEGIN
  ...
  IF in_chargeback_date IS NULL THEN
    var_chargeback_date := PROCS_COMMON_V24.NORMALIZE_DATE(current_date);
  ELSE
    var_chargeback_date := PROCS_COMMON_V24.NORMALIZE_DATE(in_chargeback_date);
  END IF;
  ...
  var_invoice_start_date := PROCS_COMMON_V24.NORMALIZE_DATE(var_invoice_start_date);
  ...
  PROCS_OFFER_CHAIN_V24.GET_OFFER_LENGTH_IN_DAYS(
  ...
  PROCS_INVOICE_V24.GET_INVOICE_DAYS_USED_NUMBER(
    in_invoice_id      => in_invoice_id,
  ...
  PROCS_OFFER_CHAIN_V24.GET_OFFER_CHAIN_MD_VALUE(
    in_offer_chain_id => var_offer_chain_id,
    in_meta_data_name => GLOBAL_CONSTANTS_V24.OFFER_CHAIN_FULL_REFUND,
  ...
  PROCS_OFFER_CHAIN_V24.GET_OFFER_CHAIN_MD_VALUE(
    in_offer_chain_id => var_offer_chain_id,
    in_meta_data_name => GLOBAL_CONSTANTS_V24.OFFER_CHAIN_PRORATED_REFUND,
    out_value         => var_offer_chain_meta_data_val
  );
  IF var_offer_chain_meta_data_val IS NULL THEN
  ...
  var_chargeback_calculated := GLOBAL_CONSTANTS_V24.FALSE;
  
  IF var_offer_chain_full_refund IS NOT NULL
     AND var_chargeback_calculated = GLOBAL_CONSTANTS_V24.FALSE THEN
    IF var_license_days_used < var_offer_chain_full_refund THEN
      out_chargeback_amount := var_invoice_amount;
      var_chargeback_calculated := GLOBAL_CONSTANTS_V24.TRUE;
    END IF;
  END IF;
  ...
  IF var_chargeback_calculated = GLOBAL_CONSTANTS_V24.TRUE THEN
    PROCS_INVOICE_V24.GET_MAX_REFUND(
      in_invoice_id => in_invoice_id,
  ...
  IF var_chargeback_calculated = GLOBAL_CONSTANTS_V24.FALSE THEN
...
BEGIN
  ...
  BEGIN
    PROCS_TRANSACTION_V24.CREATE_TRANSACTION(
      in_transaction_id         => NULL,
      in_status_id              => GLOBAL_STATUSES_V24.TRANSACTION_PREPARE,
      in_amount                 => -var_charge_amount_to_apply,
      in_created_by             => in_created_by,
      in_order_id               => NULL,
      in_is_refund              => GLOBAL_CONSTANTS_V24.TRUE,
...
BEGIN
  ...
  LOOP
    ...
    IF f_charge.CHARGE_STATUS_ID != GLOBAL_STATUSES_V24.CHARGE_CANCELED THEN
    
      IF f_charge.CHARGE_AMOUNT > 0 THEN
        IF f_charge.CHARGE_STATUS_ID IN  (GLOBAL_STATUSES_V24.CHARGE_PROCESSED, GLOBAL_STATUSES_V24.CHARGE_CAPTURED,
        GLOBAL_STATUSES_V24.CHARGE_SETTLED,GLOBAL_STATUSES_V24.CHARGE_CHARGEBACK_CLAIM, GLOBAL_STATUSES_V24.CHARGEBACK_SETTLED)
           AND PROCS_TRANSACTION_V24.IS_TRANSACTION_COLLECTED(f_charge.TRANSACTION_ID) = GLOBAL_CONSTANTS_V24.TRUE THEN
      ...
      ELSE
        IF f_charge.CHARGE_STATUS_ID IN (GLOBAL_STATUSES_V24.CHARGE_PROCESSED, GLOBAL_STATUSES_V24.CHARGE_CAPTURED,
        GLOBAL_STATUSES_V24.CHARGE_SETTLED,GLOBAL_STATUSES_V24.CHARGE_CHARGEBACK_CLAIM, GLOBAL_STATUSES_V24.CHARGEBACK_SETTLED)
           AND PROCS_TRANSACTION_V24.IS_TRANSACTION_COLLECTED(f_charge.TRANSACTION_ID) = GLOBAL_CONSTANTS_V24.FALSE THEN
...
BEGIN
  ...
  FOR f_license IN (
    ...
    WHERE
      LICENSE.INVOICE_ID = in_invoice_id
      AND LICENSE.IS_EXTENSION = GLOBAL_CONSTANTS_V24.FALSE
  )
  LOOP
    var_license_start_date := PROCS_COMMON_V24.NORMALIZE_DATE(f_license.START_DATE);
    var_license_end_date := PROCS_COMMON_V24.NORMALIZE_DATE(f_license.END_DATE);
...
BEGIN
  ...
  --    in_license_id         => f_license_to_cancel.ID,
  --    in_license_status_id  => GLOBAL_STATUSES_V24.LICENSE_CLOSED,
  --    in_needs_entitlements => GLOBAL_CONSTANTS_V24.FALSE,
  --    in_updated_by         => in_updated_by
  --  );
  --END LOOP;
...
BEGIN
  ...
  IF var_is_tax_exempt IS NULL THEN
    out_is_tax_exempt := GLOBAL_CONSTANTS_V24.FALSE;
  ELSE
    out_is_tax_exempt := GLOBAL_CONSTANTS_V24.TRUE;
  END IF;
...
BEGIN
  ...
  IF var_account_status_id = GLOBAL_STATUSES_V24.ACCOUNT_FROZEN THEN
    RAISE ACCOUNT_IS_FROZEN;
  END IF;
  
  IF var_invoice_status_id != GLOBAL_STATUSES_V24.INVOICE_CLOSED THEN
  ...
  WHERE
    CHARGE.INVOICE_ID = in_invoice_id
    AND (
      CHARGE.CHARGE_STATUS_ID = GLOBAL_STATUSES_V24.CHARGE_OPENED
      OR (
        CHARGE.CHARGE_STATUS_ID in  (GLOBAL_STATUSES_V24.CHARGE_PROCESSED, GLOBAL_STATUSES_V24.CHARGE_CAPTURED,
        GLOBAL_STATUSES_V24.CHARGE_SETTLED,GLOBAL_STATUSES_V24.CHARGE_CHARGEBACK_CLAIM, GLOBAL_STATUSES_V24.CHARGEBACK_SETTLED)
        AND EXISTS (
          SELECT 1 FROM TRANSACTION_ATTEMPT ta where ta.transaction_id = CHARGE.TRANSACTION_ID and ta.transaction_attempt_status_id = GLOBAL_STATUSES_V24.TRANS_ATTEMPT_SUCCESS
        )
      )
    )
    AND CHARGE.CHARGE_AMOUNT < 0;
  ...
  FOR f_processed_charges IN (
    ...
    WHERE
      ...
      AND CHARGE.CHARGE_STATUS_ID IN (GLOBAL_STATUSES_V24.CHARGE_PROCESSED, GLOBAL_STATUSES_V24.CHARGE_CAPTURED,
        GLOBAL_STATUSES_V24.CHARGE_SETTLED,GLOBAL_STATUSES_V24.CHARGE_CHARGEBACK_CLAIM, GLOBAL_STATUSES_V24.CHARGEBACK_SETTLED)
      AND EXISTS (SELECT 1 FROM TRANSACTION_ATTEMPT ta where ta.transaction_id = CHARGE.TRANSACTION_ID and ta.transaction_attempt_status_id = GLOBAL_STATUSES_V24.TRANS_ATTEMPT_SUCCESS)
  )
  LOOP
...
BEGIN
  ...
  where
    ch.invoice_id = in_invoice_id
    and ta.transaction_attempt_status_id = GLOBAL_STATUSES_V24.TRANS_ATTEMPT_SUCCESS
...
BEGIN
  SELECT DECODE(COUNT(1), 0, GLOBAL_CONSTANTS_V24.FALSE, GLOBAL_CONSTANTS_V24.TRUE)
    into out_is_revoke
  FROM
    offer_chain oc,
    subscription s,
...
prompt Compiling body for procs_fin_instruments.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_FIN_INSTRUMENTS_V24" AS
...
BEGIN
  FOR rec IN (SELECT id FROM Gift_Certificate WHERE Purchase_Invoice_Id = in_invoice_id) LOOP
    PROCS_FIN_INSTRUMENTS_CRU_V24.UPDATE_GIFT_CERTIFICATE (
  ...
  WHERE
    ...
    AND NOT EXISTS(
      ...
      WHERE
        ...
        ta.transaction_attempt_status_id = GLOBAL_STATUSES_V24.TRANS_ATTEMPT_SUCCESS
    ) AND NOT EXISTS (
      SELECT NULL
      FROM
        charge c,
      ...
      WHERE
        c.invoice_id = gc.purchase_invoice_id and
        c.transaction_id = t.id and
        t.is_refund = GLOBAL_CONSTANTS_V24.TRUE
    )
...
BEGIN
  ...
  PROCS_FIN_INSTRUMENTS_CRU_V24.CREATE_CREDIT_CARD(
    out_credit_card_id          => var_credit_card_id,
    in_account_id               => var_account_id,
...
BEGIN
  ...
  IF in_paypal_status_id != GLOBAL_STATUSES_V24.PAYPAL_ACTIVE
    AND in_paypal_status_id != GLOBAL_STATUSES_V24.PAYPAL_INACTIVE
    AND in_paypal_status_id != GLOBAL_STATUSES_V24.PAYPAL_FROZEN THEN
  ...
  BEGIN
    PROCS_FIN_INSTRUMENTS_CRU_V24.CREATE_PAYPAL(
      out_paypal_id                  => var_paypal_id,
      in_paypal_id                   => NULL,
      in_account_id                  => var_account_id,
      in_instrument_name             => in_instrument_name,
...
BEGIN
  ...
  IF F_CAN_DISABLE_CREDIT_CARD(in_credit_card_id) = GLOBAL_CONSTANTS_V24.FALSE THEN
  ...
  IF var_credit_card_status != GLOBAL_STATUSES_V24.CREDIT_CARD_ACTIVE THEN
    RAISE BAD_CC_STATUS;
  END IF;
  ...
  FROM
    ...
    INNER JOIN TRANSACTION ON CHARGE.TRANSACTION_ID = TRANSACTION.ID
  WHERE
    CHARGE.INSTRUMENT_TYPE_ID = GLOBAL_ENUMS_V24.INSTRUMENT_CREDIT_CARD
    AND CHARGE.INSTRUMENT_ID = in_credit_card_id
    AND TRANSACTION.TRANSACTION_STATUS_ID = GLOBAL_STATUSES_V24.TRANSACTION_PENDING;
  ...
  IF current_def_instrument_type_id = GLOBAL_ENUMS_V24.INSTRUMENT_CREDIT_CARD
  ...
  PROCS_FIN_INSTRUMENTS_V24.UPDATE_CREDIT_CARD_STATUS(
    in_credit_card_id        => in_credit_card_id,
    in_updated_by            => in_updated_by,
    in_credit_card_status_id => GLOBAL_STATUSES_V24.CREDIT_CARD_DISABLED
  );
...
BEGIN
  ...
  IF var_paypal_status_id = GLOBAL_STATUSES_V24.PAYPAL_INACTIVE THEN
  ...
  FROM
    ...
    INNER JOIN TRANSACTION ON CHARGE.TRANSACTION_ID = TRANSACTION.ID
  WHERE
    CHARGE.INSTRUMENT_TYPE_ID = GLOBAL_ENUMS_V24.INSTRUMENT_PAYPAL
  ...
  -- Checking that credit card is not default
  IF current_def_instrument_type_id = GLOBAL_ENUMS_V24.INSTRUMENT_PAYPAL
  ...
  PROCS_FIN_INSTRUMENTS_V24.UPDATE_PAYPAL_STATUS(
    in_paypal_id        => in_paypal_id,
    in_updated_by       => in_updated_by,
    in_paypal_status_id => GLOBAL_STATUSES_V24.PAYPAL_INACTIVE
  );
...
BEGIN
  ...
  IF in_is_default != GLOBAL_CONSTANTS_V24.TRUE
    AND in_is_default != GLOBAL_CONSTANTS_V24.FALSE
  ...
  IF in_instrument_name IS NOT NULL THEN
    PROCS_FIN_INSTRUMENTS_CRU_V24.UPDATE_CREDIT_CARD(
      in_credit_card_id  => in_credit_card_id,
      in_updated_by      => in_updated_by,
      in_instrument_name => in_instrument_name
    );
  ...
  IF in_is_default = GLOBAL_CONSTANTS_V24.TRUE THEN
    BEGIN
      PROCS_FIN_INSTRUMENTS_V24.SET_DEF_FINANCIAL_INSTRUMENT(
        in_group_id           => var_group_id,
        in_instrument_type_id => GLOBAL_ENUMS_V24.INSTRUMENT_CREDIT_CARD,
  ...
  -- Set default financial instrument
  IF in_is_default = GLOBAL_CONSTANTS_V24.FALSE THEN
    BEGIN
      GET_DEF_FINANCIAL_INSTRUMENT(
        in_group_id            => var_group_id,
        out_instrument_type_id => current_def_instrument_type_id,
...
-- EXCEPTIONS
      ...
      CAN_NOT_CREATE_LINE_ITEMS     EXCEPTION;
      REF_CODE_ALREADY_EXISTS       EXCEPTION;
...
-- Get offer chain flag "is_gift_certificate"
      ...
      IF var_och_is_gc != GLOBAL_CONSTANTS_V24.TRUE
...
-- Add line items for new invoice
      ...
      BEGIN
        PROCS_FIN_INSTRUMENTS_CRU_V24.CREATE_GIFT_CERTIFICATE(
            ...
            in_gift_certificate_status_id => GLOBAL_STATUSES_V24.GIFT_CERTIFICATE_ACTIVE,
        ...
        EXCEPTION WHEN DUP_VAL_ON_INDEX THEN RAISE REF_CODE_ALREADY_EXISTS;
      END;
      ...
      WHEN REF_CODE_ALREADY_EXISTS THEN
      PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR,
                                       SPROC_NAME, 'reference_code already used');
      WHEN OTHERS THEN
      PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR,
                                       SPROC_NAME, 'Unknown error', SQLERRM);
...
-- Check that instrument exists
      IF in_instrument_type_id = GLOBAL_ENUMS_V24.INSTRUMENT_CREDIT_CARD THEN
        IF IS_CREDIT_CARD_EXISTS(in_instrument_id) = GLOBAL_CONSTANTS_V24.FALSE THEN
          RAISE BAD_CREDIT_CARD_ID;
        END IF;
      ELSIF in_instrument_type_id = GLOBAL_ENUMS_V24.INSTRUMENT_PAYPAL THEN
        IF IS_PAYPAL_EXISTS(in_instrument_id) = GLOBAL_CONSTANTS_V24.FALSE THEN
          RAISE BAD_PAYPAL_ID;
        END IF;
      ELSIF in_instrument_type_id = GLOBAL_ENUMS_V24.INSTRUMENT_ZCI_INSTRUMENT THEN
        NULL;
      ELSE
        RAISE BAD_INSTRUMENT_TYPE;
      END IF;
...
-- Calculate new invoice amount
      BEGIN
        PROCS_INVOICE_V24.CALCULATE_INVOICE_AMOUNT(in_invoice_id, var_invoice_amount);
        EXCEPTION
        WHEN OTHERS THEN
        EXCEPTION_MESSAGE := SQLERRM;
        RAISE CAN_NOT_CALC_INVOICE_AMOUNT;
      ...
      IF var_invoice_amount > 0
         AND in_instrument_type_id = GLOBAL_ENUMS_V24.INSTRUMENT_ZCI_INSTRUMENT THEN
        RAISE CAN_NOT_USE_FCINSTR;
      END IF;
...
BEGIN
  ...
  -- Calculate new invoice amount
  BEGIN
    PROCS_INVOICE_V24.CALCULATE_INVOICE_AMOUNT(var_new_invoice_id, var_invoice_amount);
    EXCEPTION
      WHEN OTHERS THEN
        EXCEPTION_MESSAGE := SQLERRM;
        RAISE CAN_NOT_CALC_INVOICE_AMOUNT;
  ...
  IF var_oc_id_to_redeem IS NULL THEN
    BEGIN
      ...
      WHERE
        ...
        AND OFFER_CHAIN_META_DATA.NAME = GLOBAL_CONSTANTS_V24.REDEMPTION_OC_ID
        AND ROWNUM = 1;
...
--      AND ELIGIBILITY.NAME = GLOBAL_CONSTANTS_V24.GIFT_CERTIFICATE_REQUIRED
--      AND ELIGIBILITY.VALUE = GLOBAL_CONSTANTS_V24.ELIGIBILITY_FLAG_SET
...
--      AND ROWNUM = 1;
  ...
  IF var_gc_purchase_inv_status_id != GLOBAL_STATUSES_V24.INVOICE_CLOSED THEN
  ...
  IF var_succ_purch_attempts_num = 0 THEN
    ...
    FROM
      DUAL
    WHERE
      PROCS_INVOICE_V24.F_CALCULATE_INVOICE_AMOUNT(var_gc_purchase_invoice_id) = 0;
  END IF;
...
IF var_same_offer_chains_num = 0 THEN
    ...
    LOOP
      IF PROCS_OFFER_CHAIN_V24.CHECK_FOR_SAME_PRODUCTS(var_oc_id_to_redeem, f_account_offer_chains.OFFER_CHAIN_ID) = GLOBAL_CONSTANTS_V24.TRUE THEN
  ...
  ELSE
    ...
    -- and if MAX_CONCURRENT_SUBS <= [user's subscription count for the offer chain]
    -- then deny purchase
    var_max_concurrent_subscrs := PROCS_OFFER_CHAIN_V24.GET_OFFER_CHAIN_MAX_CONC_SUBSC(var_oc_id_to_redeem);
    IF var_max_concurrent_subscrs != GLOBAL_CONSTANTS_V24.INFINITY
      AND var_max_concurrent_subscrs <= var_same_offer_chains_num THEN
      RAISE LIMIT_REACHED;
    END IF;
...
-- norlov: END OF TODO
  ...
  END IF;
   ...
   IF var_gc_status_id = GLOBAL_STATUSES_V24.GIFT_CERTIFICATE_REFUNDED THEN
    RAISE GIFT_CERT_IS_REFUNDED;
  END IF;

  IF var_gc_status_id = GLOBAL_STATUSES_V24.GIFT_CERTIFICATE_FINALIZED THEN
  ...
  LOOP
    IF PROCS_OFFER_CHAIN_V24.CHECK_FOR_SAME_PRODUCTS(var_oc_id_to_redeem, f_user_offer_chain.OFFER_CHAIN_ID)=GLOBAL_CONSTANTS_V24.TRUE THEN
      RAISE USER_ALREADY_SUBSCRIBED_TO_PRD;
    END IF;
  END LOOP;
  ...
  IF var_fin_instrument_id is null THEN
    ...
    var_fin_instrument_type_id := GLOBAL_ENUMS_V24.INSTRUMENT_GIFT_CERTIFICATE;
  END IF;
  ...
  BEGIN
    PROCS_OFFER_CHAIN_V24.GET_FIRST_OFFER(var_oc_id_to_redeem, var_first_offer_id);
    EXCEPTION
      WHEN OTHERS THEN
        EXCEPTION_MESSAGE := SQLERRM;
        RAISE CAN_NOT_GET_FIRST_OFFER_CHAIN;
  ...
  BEGIN
    PROCS_LICENSE_V24.CREATE_LICENSE (
      ...
      in_end_date                 => NULL, -- Will be calculated automatically
      in_is_extension             => GLOBAL_CONSTANTS_V24.FALSE,
      in_current_offer_index      => PROCS_OFFER_CHAIN_V24.GET_FIRST_OFFER_INDEX(var_oc_id_to_redeem),
      in_current_offer_recurr_num => 1
    );
  ...
  BEGIN
    PROCS_FIN_INSTRUMENTS_CRU_V24.UPDATE_GIFT_CERTIFICATE(
      ...
      in_gift_certificate_status_id => GLOBAL_STATUSES_V24.GIFT_CERTIFICATE_FINALIZED
    );
    EXCEPTION
      WHEN OTHERS THEN
        EXCEPTION_MESSAGE := SQLERRM;
...
BEGIN
  ...
  LEFT JOIN GC_CAMPAIGN_AND_REF gccr ON gc.id = gccr.GC_ID
  WHERE
    gc.code = in_code;
...
BEGIN
  ...
  PROCS_ACCOUNT_CRU_V24.UPDATE_ACCOUNT(
    in_account_id         => var_account_id,
    in_updated_by         => in_updated_by,
    in_instrument_type_id => in_instrument_type_id,
    in_instrument_id      => in_instrument_id
...
BEGIN
  ...
  PROCS_ACCOUNT_CRU_V24.UPDATE_DEF_FIN_INSTRUMENT(
    in_account_id => var_account_id,
...
SPROC_NAME    CONSTANT VARCHAR2(32) := 'GET_CREDIT_CARD_BY_ID';
BEGIN
  OPEN out_result_set FOR
  SELECT
    ID,
...
SPROC_NAME CONSTANT VARCHAR2(32) := 'GET_PAYPAL_BY_ID';
BEGIN
  OPEN out_result_set FOR
  SELECT
    ID,
...
FUNCTION IS_CREDIT_CARD_EXISTS (
/*
GLOBAL_CONSTANTS_V24.TRUE - if instrument exists
GLOBAL_CONSTANTS_V24.FALSE - else
*/
  in_credit_card_id IN NUMBER
) RETURN NUMBER AS
...
BEGIN
  ...
  IF in_credit_card_status_id != GLOBAL_STATUSES_V24.CREDIT_CARD_ACTIVE
    AND in_credit_card_status_id != GLOBAL_STATUSES_V24.CREDIT_CARD_INVALID
    AND in_credit_card_status_id != GLOBAL_STATUSES_V24.CREDIT_CARD_DISABLED
    AND in_credit_card_status_id != GLOBAL_STATUSES_V24.CREDIT_CARD_EXPIRED THEN
    RAISE BAD_STATUS_ID;
  END IF;
...
BEGIN
  ...
  PROCS_FIN_INSTRUMENTS_CRU_V24.UPDATE_PAYPAL(
    in_paypal_id        => in_paypal_id,
    in_paypal_status_id => in_paypal_status_id,
    in_updated_by       => in_updated_by
  );
...
BEGIN
  ...
  IF in_gift_certificate_status_id != GLOBAL_STATUSES_V24.GIFT_CERTIFICATE_ACTIVE
    AND in_gift_certificate_status_id != GLOBAL_STATUSES_V24.GIFT_CERTIFICATE_FINALIZED THEN
    RAISE BAD_STATUS_ID;
  END IF;
...
BEGIN
  ...
  IF in_old_fin_instrument_type = GLOBAL_ENUMS_V24.INSTRUMENT_CREDIT_CARD THEN
    IF PROCS_FIN_INSTRUMENTS_V24.IS_CREDIT_CARD_EXISTS(in_old_fin_instrument_id) = GLOBAL_CONSTANTS_V24.FALSE THEN
      -- throw exception: bad old credit card
      RAISE BAD_OLD_CC;
    END IF;
  ELSIF in_old_fin_instrument_type = GLOBAL_ENUMS_V24.INSTRUMENT_PAYPAL THEN
    IF PROCS_FIN_INSTRUMENTS_V24.IS_PAYPAL_EXISTS(in_old_fin_instrument_id) = GLOBAL_CONSTANTS_V24.FALSE THEN
  ...
  IF in_new_fin_instrument_type = GLOBAL_ENUMS_V24.INSTRUMENT_CREDIT_CARD THEN
    IF PROCS_FIN_INSTRUMENTS_V24.IS_CREDIT_CARD_EXISTS(in_new_fin_instrument_id) = GLOBAL_CONSTANTS_V24.FALSE THEN
      -- throw exception: bad new credit card
      RAISE BAD_NEW_CC;
    END IF;
  ELSIF in_new_fin_instrument_type = GLOBAL_ENUMS_V24.INSTRUMENT_PAYPAL THEN
    IF PROCS_FIN_INSTRUMENTS_V24.IS_PAYPAL_EXISTS(in_new_fin_instrument_id) = GLOBAL_CONSTANTS_V24.FALSE THEN
  ...
  SELECT count(1) into var_accounts_count FROM (
    ...
    WHERE
      (
        ...
        AND in_old_fin_instrument_type = GLOBAL_ENUMS_V24.INSTRUMENT_CREDIT_CARD
      )
      OR
      (
        CC.ID = in_new_fin_instrument_id
        AND in_new_fin_instrument_type = GLOBAL_ENUMS_V24.INSTRUMENT_CREDIT_CARD
      )
    UNION
    SELECT
      PP.ACCOUNT_ID
    ...
    WHERE
      (
        ...
        AND in_old_fin_instrument_type = GLOBAL_ENUMS_V24.INSTRUMENT_PAYPAL
      )
      OR
      (
        PP.ID = in_new_fin_instrument_id
        AND in_new_fin_instrument_type = GLOBAL_ENUMS_V24.INSTRUMENT_PAYPAL
      )
  )
  WHERE
  ...
  FOR f_sub IN (
    ...
    WHERE
      (
        s.subscription_status_id = GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE OR
        s.subscription_status_id = GLOBAL_STATUSES_V24.SUBSCRIPTION_SUSPENDED OR
        s.subscription_status_id = GLOBAL_STATUSES_V24.SUBSCRIPTION_IN_GRACE_PERIOD
      )
      AND
  ...
  FOR f_open_charge IN (
    ...
    WHERE
      ...
      AND ch.charge_status_id = GLOBAL_STATUSES_V24.CHARGE_OPENED
  ) LOOP
    ...
    FOR f_pending_transaction IN (
      ...
      where
        ...
        and transaction_status_id = GLOBAL_STATUSES_V24.TRANSACTION_PENDING
    ) LOOP
      ...
      PROCS_ADJUSTMENTS_V24.UPDATE_INVOICE_ADJUSTMENT(
        IN_INVOICE_ID => f_open_charge.invoice_id,
...
/*
    Note, this returns paypal records which are in the given window of time
    based on their creation date, not their expiration date, because we've seen
    suspicious values in the data for expiration dates. The java layer should
    inspect the expiration dates and decide whether or not to act on the data.
*/
...
SPROC_NAME CONSTANT VARCHAR2(32) := 'GET_EXPIRING_PAYPAL';
var_rows_to_return CONSTANT NUMBER := 1000;
BEGIN
  OPEN out_result_set FOR
  SELECT * FROM (
        SELECT
        ...
        pp.create_date      AS paypal_creation_date,
        pp.expiration_date  AS paypal_expiration_date,
        lc.end_date         AS license_end_date
    FROM license lc
    INNER JOIN subscription sb
    ON lc.subscription_id = sb.id
    INNER JOIN paypal pp
    ON pp.account_id = sb.account_id
    INNER JOIN account ac
    ON pp.account_id = ac.id
    WHERE SYSDATE <= lc.end_date
      AND SYSDATE + in_expire_window_days > lc.end_date
      AND pp.paypal_status_id = GLOBAL_STATUSES_V24.PAYPAL_ACTIVE
      AND lc.license_status_id IN (GLOBAL_STATUSES_V24.LICENSE_ACTIVE,
                                   GLOBAL_STATUSES_V24.LICENSE_IN_GRACE_PERIOD)
      AND pp.id = sb.instrument_id
      AND sb.instrument_type_id = GLOBAL_ENUMS_V24.INSTRUMENT_PAYPAL
      AND sb.subscription_status_id IN (GLOBAL_STATUSES_V24.SUBSCRIPTION_ACTIVE,
                                        GLOBAL_STATUSES_V24.SUBSCRIPTION_IN_GRACE_PERIOD)
      AND pp.create_date + in_creation_limit_days <= lc.end_date
      AND NOT EXISTS(
        SELECT NULL
        FROM process_retry_throttle rt
        WHERE rt.process_name = in_retry_throttle_name
          AND rt.generic_id = pp.id
      )
      AND ROWNUM <= 10 * var_rows_to_return
    ORDER BY dbms_random.value
  )
  WHERE ROWNUM <= var_rows_to_return;
END GET_EXPIRING_PAYPAL;
...
prompt Compiling body for procs_group_account.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_GROUP_ACCOUNT_V24" AS
...
BEGIN
  CORE_OWNER.PROCS_GROUP_ACCOUNT_CRU_V24.UPDATE_SUBSCRIPTION_SHARE (
...
BEGIN
  ...
  SELECT * FROM (
    ...
    WHERE
      ...
      AND Ss.Needs_Entitlements  = GLOBAL_CONSTANTS_V24.TRUE
...
BEGIN
  ...
  upper_bond_diff :=  range_diff - GLOBAL_CONSTANTS_V24.MAX_RETURN_COUNT;
...
BEGIN
  ...
  WHERE
   ...
   GROUP_ACC_IP_RNG_STATUS_ID = GLOBAL_STATUSES_V24.GROUP_ACC_IP_RNG_ACTIVE;
EXCEPTION
WHEN OTHERS THEN
  PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR,
...
BEGIN
  ...
  CORE_OWNER.PROCS_GROUP_ACCOUNT_CRU_V24.CREATE_GROUP_ACCOUNT(
    in_subscription_id => in_subscription_id,
...
SPROC_NAME CONSTANT VARCHAR2(32) := 'DISABLE_EMAIL_DOMAIN_BY_GA_ID';
BEGIN
    PROCS_GROUP_ACCOUNT_CRU_V24.DISABLE_EMAIL_DOMAIN_BY_GA_ID(
...
SPROC_NAME CONSTANT VARCHAR2(32) := 'DISABLE_EMAIL_DOMAIN_BY_ID';
BEGIN
    PROCS_GROUP_ACCOUNT_CRU_V24.DISABLE_EMAIL_DOMAIN_BY_ID(
      in_id => in_id,
      in_updated_by => in_updated_by
    );
EXCEPTION
...
BEGIN
    ...
    PROCS_GROUP_ACCOUNT_CRU_V24.ADD_EMAIL_DOMAIN(
       in_group_account_id => in_group_account_id,
       in_email_domain => in_email_domain,
       in_is_active => GLOBAL_STATUSES_V24.GROUP_ACC_EMAIL_DOMAIN_ACT,
       in_created_by => in_created_by
    );
...
WHEN OTHERS THEN
  IF (sqlcode=APP_EXCEPTION_CODES_V24.DUPLICATE_ERROR) THEN
     RAISE;
  ELSE
    PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR,
      SPROC_NAME, 'Unknown error', SQLERRM);
...
BEGIN
  start_date := sysdate;
  end_date   := GLOBAL_CONSTANTS_V24.MAX_DATE;
  ...
  CORE_OWNER.PROCS_GROUP_ACCOUNT_CRU_V24.CREATE_SUBSCRIPTION_SHARE(
    in_group_account_id => in_group_account_id,
...
BEGIN
  SELECT 
    PROCS_GROUP_ACCOUNT_V24.F_GET_NUM_OCCUPIED_GROUP_SEATS(in_group_account_id) INTO out_occupied_seats
  FROM dual;
EXCEPTION
WHEN OTHERS THEN
  PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR,
...
SPROC_NAME CONSTANT VARCHAR2(32) := 'DISABLE_IP_RANGES_BY_GA_ID';
BEGIN
    PROCS_GROUP_ACCOUNT_CRU_V24.DISABLE_IP_RANGES_BY_GA_ID(
      in_group_account_id => in_group_account_id,
      in_updated_by => in_updated_by
    );
EXCEPTION
...
SPROC_NAME CONSTANT VARCHAR2(32) := 'DISABLE_IP_RANGE_BY_ID';
BEGIN
    PROCS_GROUP_ACCOUNT_CRU_V24.DISABLE_IP_RANGE_BY_ID(
      in_id => in_id,
      in_updated_by => in_updated_by
    );
EXCEPTION
WHEN OTHERS THEN
  IF SQLCODE = APP_EXCEPTION_CODES_V24.INVALID_PARAMETER THEN
    RAISE;
  ELSE
    PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR,
    SPROC_NAME, 'Unknown error', SQLERRM);
...
BEGIN
    ...
    PROCS_GROUP_ACCOUNT_CRU_V24.ADD_IP_RANGE(
      in_group_account_id => in_group_account_id,
    ...
    FROM
      GROUP_ACCOUNT_IP_RANGE
    WHERE
      GROUP_ACC_IP_RNG_STATUS_ID = GLOBAL_STATUSES_V24.GROUP_ACC_IP_RNG_ACTIVE AND
          ((
...
BEGIN
  PROCS_GROUP_ACCOUNT_CRU_V24.UPDATE_GROUP_ACCOUNT(
    in_group_account_id => in_group_account_id,
    in_group_name => in_group_name,
    in_first_name => in_first_name,
    in_last_name => in_last_name,
...
BEGIN
  PROCS_GROUP_ACCOUNT_CRU_V24.UPDATE_GROUP_ACCOUNT_SEATS(
    in_group_account_id => in_group_account_id,
    in_seats => in_seats,
    in_updated_by => in_updated_by
  );
...
prompt Compiling body for procs_test.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_TEST_V24" AS
...
BEGIN
   ...
   OPEN out_result_set FOR SELECT 
      ...
      INVOICE.INVOICE_STATUS_ID,
      PROCS_INVOICE_V24.F_CALCULATE_INVOICE_AMOUNT(in_invoice_id) AS "INVOICE_AMOUNT",
      CHARGE.ID AS "CHARGE_ID",
      CHARGE.CHARGE_AMOUNT,
      CHARGE.TRANSACTION_ID,
      TRANSACTION.TRANSACTION_STATUS_ID
...
prompt Compiling body for procs_account.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_ACCOUNT_V24" AS
...
BEGIN
  ...
  Where
    ...
    INVOICE.INVOICE_STATUS_ID = GLOBAL_STATUSES_V24.INVOICE_OPEN;
...
BEGIN
  ...
  PROCS_ACCOUNT_CRU_V24.CREATE_ACCOUNT_NOTE(
...
BEGIN
  ...
  out_exists := GLOBAL_CONSTANTS_V24.TRUE;
EXCEPTION
WHEN NO_DATA_FOUND THEN
  out_exists := GLOBAL_CONSTANTS_V24.FALSE;
WHEN OTHERS THEN
  PROCS_COMMON_V24.THROW_EXCEPTION(APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR,
    SPROC_NAME, 'Unknown error', SQLERRM);
...
BEGIN
  ...
  IF current_account_status = GLOBAL_STATUSES_V24.ACCOUNT_DISABLED THEN
  ...
  WHERE
    SUBSCRIPTION.ACCOUNT_ID = var_account_id
    AND INVOICE.INVOICE_STATUS_ID IN ( SELECT GLOBAL_STATUSES_V24.INVOICE_OPEN FROM DUAL );
  ...
  PROCS_ACCOUNT_V24.UPDATE_ACCOUNT_STATUS(
    in_account_id        => var_account_id,
    in_account_status_id => GLOBAL_STATUSES_V24.ACCOUNT_DISABLED,
    in_updated_by        => in_updated_by
  );
  ...
  IF in_note IS NOT NULL THEN
    BEGIN
      PROCS_ACCOUNT_V24.ANNOTATE_ACCOUNT(
...
SPROC_NAME CONSTANT VARCHAR2(32) := 'CREATE_ACTIVE_ACCOUNT';
var_count  NUMBER  := 0;
BEGIN
  SELECT COUNT(1) INTO var_count FROM ACCOUNT WHERE GROUP_ID = in_group_id;

  IF (var_count = 0) THEN
    PROCS_ACCOUNT_CRU_V24.CREATE_ACCOUNT(
        out_account_id        => out_new_account_id,
        in_account_status_id  => GLOBAL_STATUSES_V24.ACCOUNT_ACTIVE,
        in_group_id           => in_group_id,
        in_created_by         => in_created_by,
        in_system_category_id => GLOBAL_ENUMS_V24.SYSTEM_CATEGORY_LIVE
    );
  ELSE
    out_new_account_id := NULL;
  END IF;
...
BEGIN
  ...
  IF current_account_status != GLOBAL_STATUSES_V24.ACCOUNT_FROZEN THEN
    RAISE BAD_CURRENT_ACC_STATUS;
  END IF;
...
BEGIN
  ...
  -- Check that incoming data is correct
  IF in_instr_status != GLOBAL_CONSTANTS_V24.TRUE AND in_instr_status != GLOBAL_CONSTANTS_V24.FALSE THEN
  ...
  WHERE
    GIFT_CERTIFICATE.PURCHASER_GROUP_ID = in_group_id
    AND (
          ( in_instr_status = GLOBAL_CONSTANTS_V24.TRUE AND
            (
              GIFT_CERTIFICATE.EXPIRATION_DATE >= current_date
              AND GIFT_CERTIFICATE.GIFT_CERTIFICATE_STATUS_ID = GLOBAL_STATUSES_V24.GIFT_CERTIFICATE_ACTIVE
            )
          )
          OR
          (
            in_instr_status = GLOBAL_CONSTANTS_V24.FALSE AND
            (
              GIFT_CERTIFICATE.EXPIRATION_DATE < current_date
              OR GIFT_CERTIFICATE.GIFT_CERTIFICATE_STATUS_ID != GLOBAL_STATUSES_V24.GIFT_CERTIFICATE_ACTIVE
            )
          )
        );
...
BEGIN
  ...
  WHERE
    SUBSCRIPTION.ACCOUNT_ID = var_account_id
    AND (SUBSCRIPTION.SCT_ID IS NULL OR SUBSCRIPTION.SCT_ID != GLOBAL_STATUSES_V24.REAL_TIME_CANCEL_REASON)
    AND SUBSCRIPTION.SUBSCRIPTION_STATUS_ID = NVL(in_status, SUBSCRIPTION.SUBSCRIPTION_STATUS_ID)
    AND PROCS_COMMON_V24.NORMALIZE_DATE(SUBSCRIPTION.PURCHASE_DATE) >= NVL(in_start_date, PROCS_COMMON_V24.NORMALIZE_DATE(SUBSCRIPTION.PURCHASE_DATE))
    AND PROCS_COMMON_V24.NORMALIZE_DATE(SUBSCRIPTION.PURCHASE_DATE) <= NVL(in_end_date, PROCS_COMMON_V24.NORMALIZE_DATE(SUBSCRIPTION.PURCHASE_DATE))
...
BEGIN
  ...
  IF var_account_status_id != GLOBAL_STATUSES_V24.ACCOUNT_ACTIVE
    AND var_account_status_id != GLOBAL_STATUSES_V24.ACCOUNT_FROZEN THEN
  ...
  PROCS_ACCOUNT_V24.UPDATE_ACCOUNT_STATUS(
    in_account_id        => var_account_id,
    in_updated_by        => in_updated_by,
    in_account_status_id => GLOBAL_STATUSES_V24.ACCOUNT_FROZEN
  );
...
BEGIN
  ...
  SELECT DISTINCT
    ...
    INVOICE.CREATE_DATE,
    INVOICE.INVOICE_STATUS_ID,
    PROCS_INVOICE_V24.F_CALCULATE_INVOICE_AMOUNT(INVOICE.ID) as "AMOUNT",
    OFFER_CHAIN.ID as "OFFER_CHAIN_ID",
    OFFER_CHAIN.NAME as "OFFER_CHAIN_NAME",
    SUBSCRIPTION.ID as "SUBSCRIPTION_ID",
    NULL as "GC_CODE",
...
BEGIN
  ...
  IF in_account_status_id != GLOBAL_STATUSES_V24.ACCOUNT_ACTIVE
    AND in_account_status_id != GLOBAL_STATUSES_V24.ACCOUNT_FROZEN
    AND in_account_status_id != GLOBAL_STATUSES_V24.ACCOUNT_DISABLED THEN
    RAISE BAD_STATUS_ID;
  END IF;
...
BEGIN
  ...
  WHERE
    SUBSCRIPTION.ACCOUNT_ID = var_account_id
    AND LICENSE.NEEDS_ENTITLEMENTS = GLOBAL_CONSTANTS_V24.TRUE;
...
BEGIN
  ...
  FROM
    ...
    INNER JOIN OFFER_CHAIN_META_DATA OCHMD ON (OCHMD.NAME = GLOBAL_CONSTANTS_V24.OCMD_UPGRADABLE_OFFER_CHAIN_ID AND TO_NUMBER(OCHMD.VALUE) = OCH.ID)
  WHERE
    S.ACCOUNT_ID = var_account_id;
...
prompt Compiling body for procs_process_retry.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_PROCESS_RETRY_V24" AS
...
SPROC_NAME CONSTANT VARCHAR2(32) := 'LOG_RETRY_V24';
BEGIN
...
WHEN OTHERS THEN
  rollback;
  PROCS_COMMON_V24.THROW_EXCEPTION(CORE_OWNER.APP_EXCEPTION_CODES_V24.UNKNOWN_ERROR,
    SPROC_NAME, 'Unknown error', SQLERRM);
...
SPROC_NAME CONSTANT VARCHAR2(32) := 'LOG_RETRY_DATE_V24';
BEGIN
...
where
  ...
  create_date <= sysdate - (in_remove_minutes / (60 * 24));
commit;
END;
...
prompt Compiling body for procs_notification.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_NOTIFICATION_V24" AS
...
EXCEPTION
WHEN NO_DATA_FOUND THEN
  PROCS_COMMON_V24.THROW_EXCEPTION(CORE_OWNER.APP_EXCEPTION_CODES_V24.NOT_FOUND,
...
BEGIN
  ...
  INSERT INTO NOTIFICATION (
    ...
    NOTIFICATION_STATUSES_V24.NOTIFICATION_PENDING,
...
END ADD_SINGLE_NOTIFICATION_ROW;
...
var_assoc_hermes_id NUMBER;
BEGIN
  CORE_OWNER.PROCS_NOTIFICATION_V24.ADD_SINGLE_NOTIFICATION_ROW (
    in_sender_account_id        => in_sender_account_id,
    in_recipient_group_id       => in_recipient_group_id,
    in_notification_type_id     => in_notification_type_id,
    in_date_to_notify           => in_date_to_notify,
    in_email_template_params    => in_email_template_params
  );

  SELECT notification_type.assoc_hermes_id INTO var_assoc_hermes_id
  FROM notification_type
  WHERE notification_type.id = in_notification_type_id;

  IF var_assoc_hermes_id IS NOT NULL THEN
    CORE_OWNER.PROCS_NOTIFICATION_V24.ADD_SINGLE_NOTIFICATION_ROW (
      in_sender_account_id        => in_sender_account_id,
      in_recipient_group_id       => in_recipient_group_id,
      in_notification_type_id     => var_assoc_hermes_id,
      in_date_to_notify           => in_date_to_notify,
      in_email_template_params    => in_email_template_params
    );
  END IF;
...
(
  ...
  WHERE
    ...
    AND (
      NOTIFICATION.NOTIFICATION_STATUS_ID = NOTIFICATION_STATUSES_V24.NOTIFICATION_PENDING
      OR NOTIFICATION.NOTIFICATION_STATUS_ID = NOTIFICATION_STATUSES_V24.NOTIFICATION_FAILED
    )
    AND (
...
BEGIN
  ...
  IF in_notification_status_id != NOTIFICATION_STATUSES_V24.NOTIFICATION_SENT
    AND in_notification_status_id != NOTIFICATION_STATUSES_V24.NOTIFICATION_PENDING
    AND in_notification_status_id != NOTIFICATION_STATUSES_V24.NOTIFICATION_FAILED THEN
  ...
  BEGIN
    CORE_HIST_OWNER.PUBLIC_PROCS_CORE_V24.CREATE_NOTIFICATION_HISTORY (
  ...
  IF (in_notification_status_id = NOTIFICATION_STATUSES_V24.NOTIFICATION_SENT OR num_fails >= max_fails) then
    FOR REC IN (
        ...
        BEGIN
          CORE_HIST_OWNER.PUBLIC_PROCS_CORE_V24.CREATE_NOTIF_FAILURE_HISTORY(
...
WHEN BAD_NOTIFICATION_ID THEN
  PROCS_COMMON_V24.THROW_EXCEPTION(CORE_OWNER.APP_EXCEPTION_CODES_V24.NOT_FOUND,
    SPROC_NAME, 'No such notification');
WHEN CAN_NOT_CREATE_HISTORY THEN
  PROCS_COMMON_V24.THROW_EXCEPTION(CORE_OWNER.APP_EXCEPTION_CODES_V24.INTERNAL_ERROR,
    SPROC_NAME, 'Could not create history', EXCEPTION_MESSAGE);
...
BEGIN
  ...
  SELECT
    ...
    notification.group_id,
    notification.email_template_params,
    notification_status.value AS "notification_status",
    notification_type.value AS "notification_type",
    notification_type.template_url,
    notification_hermes_data.copy AS "copy",
    notification_hermes_data.link_ref AS "link_ref",
    notification_hermes_data.link_offset AS "link_offset",
    notification_hermes_data.link_count AS "link_count"
  FROM
    notification
    INNER JOIN notification_type
    ON notification.notification_type_id = notification_type.id
    INNER JOIN notification_status
    ON notification.notification_status_id = notification_status.id
    LEFT OUTER JOIN notification_hermes_data
    ON notification_type.notification_hermes_data_id = notification_hermes_data.id
  WHERE
    notification.id = in_notification_id;
...
prompt Compiling body for public_procs_billing.sql
CREATE OR REPLACE PACKAGE BODY "PUBLIC_PROCS_BILLING_V24" AS
...
BEGIN
  PROCS_OFFER_CHAIN_V24.GET_OFFER_CHAIN_BY_ID (
    in_offer_chain_id => in_offer_chain_id,
    out_result_set => out_result_set
  );
END GET_OFFER_CHAIN_BY_ID;
...
BEGIN
  PROCS_INVOICE_V24.GET_PENDING_INVOICES(
    out_result_set,
    in_row_number
  );
END GET_PENDING_INVOICES;
...
BEGIN
  PROCS_CHARGE_V24.GET_PENDING_REFUND_CHARGES(
    out_result_set,
    in_row_number
  );
END GET_PENDING_REFUND_CHARGES;
...
BEGIN
  PROCS_CHARGE_V24.GET_UNPROCESSED_CHARGES(
    in_invoice_id,
    out_result_set
  );
END GET_UNPROCESSED_CHARGES;
...
BEGIN
  PROCS_TRANSACTION_V24.GET_TRNSCTN_ATTEMPTS_BY_STATUS(
    in_transaction_id,
...
BEGIN
  PROCS_TRANSACTION_V24.UPDATE_TRNSCTN_ATTEMPT_STATUS(
...
BEGIN
  PROCS_TRANSACTION_V24.GET_CLOSED_REFUNDS_BY_INVOICE(
     in_invoice_id,
     out_result_set
  );
END GET_CLOSED_REFUNDS_BY_INVOICE;
...
BEGIN
  PROCS_TRANSACTION_V24.UPDATE_TRNSCTN_ATTEMPT_TIME(
    in_transaction_attempt_id,
    in_updated_by
  );
END UPDATE_TRNSCTN_ATTEMPT_TIME;
...
BEGIN
  PROCS_TRANSACTION_V24.CREATE_TRANSACTION_ATTEMPT(
    in_transaction_id,
...
BEGIN
  PROCS_TRANSACTION_V24.UPDATE_TRANSACTION_ATTEMPT_INF(
    in_transaction_attempt_id,
    in_ext_status_code,
...
BEGIN
  PROCS_TRANSACTION_V24.GET_FAILED_ATTEMPTS_NUMBER(
    in_transaction_id,
    out_attempts_num
  );
END GET_FAILED_ATTEMPTS_NUMBER;
...
BEGIN
  PROCS_TRANSACTION_V24.IS_TRANSACTION_SUCCESSFULL(
    in_transaction_id,
    out_is_successfull
  );
END IS_TRANSACTION_SUCCESSFULL;
...
BEGIN
  PROCS_SUBSCRIPTION_V24.SUSPEND_SUBSCRIPTION(
    in_subs_id,
    in_updated_by
  );
END SUSPEND_SUBSCRIPTION;
...
BEGIN
  PROCS_FIN_INSTRUMENTS_V24.GET_CREDIT_CARD_BY_ID(
    in_credit_card_id,
    out_result_set
  );
END GET_CREDIT_CARD_BY_ID;
...
BEGIN
  PROCS_TRANSACTION_V24.GET_TRANSACTION_AMOUNT(
    in_transaction_id,
...
BEGIN
  PROCS_INVOICE_V24.GET_ACCOUNT_BY_INVOICE_ID(
    in_invoice_id,
    out_account_id
  );
END GET_ACCOUNT_BY_INVOICE_ID;
...
BEGIN
  PROCS_FIN_INSTRUMENTS_V24.GET_GIFT_CERTIFICATE_BY_ID (
    in_gift_certificate_id,
    out_result_set
  );
END GET_GIFT_CERTIFICATE_BY_ID;
...
BEGIN
  PROCS_CHARGE_V24.GET_SUBSCR_ID_BY_CHARGE_ID(
    in_charge_id,
    out_subscription_id
  );
END GET_SUBSCR_ID_BY_CHARGE_ID;
...
BEGIN
  PROCS_FIN_INSTRUMENTS_V24.IS_GCERT_FOR_PROPER_OFFER (
    in_gift_certificate_id,
    in_charge_id,
    out_result
  );
...
BEGIN
  PROCS_SUBSCRIPTION_V24.GET_SUBSCRIPTION_INFO (
    in_subscription_id,
    out_result_set
  );
END GET_SUBSCRIPTION_INFO;
...
BEGIN
  PROCS_TRANSACTION_V24.GET_TRANSACTION_BY_ID(
    in_transaction_id,
    out_result_set
  );
END GET_TRANSACTION_BY_ID;
...
BEGIN
  PROCS_FIN_INSTRUMENTS_V24.GET_GC_BY_PURCH_INVOICE_ID(
    in_invoice_id,
    out_result_set 
  );
END GET_GC_BY_PURCH_INVOICE_ID;
...
BEGIN
  PROCS_TRANSACTION_V24.UPDATE_TRANSACTION_ORDER_ID(
    in_transaction_id,
    in_order_id,
    in_updated_by
  );
...
BEGIN
  PROCS_SUBSCRIPTION_V24.GET_ACTIVE_INVOICES_IDS(
    in_subscription_id, 
    out_result_set
  );
END GET_ACTIVE_INVOICES_IDS;
...
BEGIN
  PROCS_SUBSCRIPTION_V24.CANCEL_SUBSCRIPTION_INVOICE(
    in_invoice_id,
    in_updated_by,
    in_refundable
  );
...
BEGIN
  PROCS_SUBSCRIPTION_V24.FINALIZE_CANCELATION(
    in_subscription_id,
...
BEGIN
  PROCS_SUBSCRIPTION_V24.GET_SUBSCR_PROD_OFFERRINGS(
    in_subscription_id,
    out_result_set
  );
END GET_SUBSCR_PROD_OFFERRINGS;
...
BEGIN
  PROCS_OFFER_CHAIN_V24.GET_OFFER_CHAIN_META_DATA(
    in_offer_chain_id,
    in_meta_data_name,
    out_result_set
  );
...
BEGIN
  PROCS_ACCOUNT_CRU_V24.READ_ACCOUNT(
    in_account_id,
    out_result_set
  );
END READ_ACCOUNT;
...
BEGIN
  PROCS_CHARGE_V24.GET_COLLECTED_CHARGES(
    in_invoice_id,
    out_result_set
  );
END GET_COLLECTED_CHARGES;
...
BEGIN
  ...
  PROCS_ACCOUNT_V24.GET_GROUPS_ID_BY_INVOICE_ID(
    in_invoice_id,
    out_group_ids
  );
...
BEGIN
  ...
  PROCS_ACCOUNT_V24.GET_ACCOUNT_ID_BY_GROUP_ID(
    in_group_id,
    out_account_id
  );
...
BEGIN
  PROCS_LOCKING_V24.LOCK_ACCOUNT(
    in_group_id,
    in_lock_key,
    in_seconds_num,
    in_created_by,
...
BEGIN
  PROCS_LOCKING_V24.RELEASE_LOCK(
    in_group_id,
    in_lock_key
  );
END RELEASE_LOCK;
...
BEGIN
  PROCS_INVOICE_V24.GET_PAYMENT_INFO_BY_INVOICE_ID(
    in_invoice_id,
    out_order_id,
    out_external_transaction_id
  );
...
BEGIN
  PROCS_FIN_INSTRUMENTS_V24.GET_PAYPAL_BY_ID(
    in_paypal_id,
    out_result_set
  );
END GET_PAYPAL_BY_ID;
...
begin
  PROCS_TRANSACTION_V24.GET_NEXT_ATTEMPT_NUMBER(
    in_charge_id,
    out_attempt_count
  );
end GET_NEXT_ATTEMPT_NUMBER;
...
begin
  PROCS_OFFER_CHAIN_V24.GET_NOTIFICATION_TYPE_ID(
    in_offer_chain_id,
    in_action_name,
...
BEGIN
PROCS_FIN_INSTRUMENTS_V24.GET_GC_ID_BY_PURCH_INVOICE_ID(
in_invoice_id,
...
BEGIN
  PROCS_SUBSCRIPTION_V24.START_GRACE_BY_INVOICE_ID(
    in_invoice_id        => in_invoice_id,
...
BEGIN
  PROCS_SUBSCRIPTION_V24.STOP_GRACE_BY_INVOICE_ID(
    in_invoice_id => in_invoice_id,
    in_updater    => in_updated_by
  );
...
prompt Compiling body for public_procs_notification.sql
CREATE OR REPLACE PACKAGE BODY "PUBLIC_PROCS_NOTIFICATION_V24" AS
...
prompt Compiling body for public_procs_renewal.sql
CREATE OR REPLACE PACKAGE BODY "PUBLIC_PROCS_RENEWAL_V24" AS

PROCEDURE SUB_EXPIRES_NEED_ENTITLEMENTS (
  out_result_set OUT SYS_REFCURSOR
) AS
BEGIN
  PROCS_GROUP_ACCOUNT_V24.SUB_EXPIRES_NEED_ENTITLEMENTS(out_result_set => out_result_set);
END SUB_EXPIRES_NEED_ENTITLEMENTS;
...
BEGIN
  PROCS_GROUP_ACCOUNT_V24.UPDATE_SS_NEED_ENTITLEMENTS(
...
BEGIN
  PROCS_LICENSE_V24.GET_ENDING_LICENSES(in_hours_number,out_result_set);
END GET_ENDING_LICENSES;
...
BEGIN
  PROCS_LICENSE_V24.GET_ENDING_LICENSES_CC(in_hours_number,out_result_set, in_process_name);
END GET_ENDING_LICENSES_CC;
...
BEGIN
  PROCS_LICENSE_V24.GET_RECURRING_OFFER (
    in_license_id,
    out_result_set
  );
END GET_RECURRING_OFFER;
...
BEGIN
  PROCS_LICENSE_V24.GET_NEXT_OFFER (
    in_license_id,
    out_result_set
  );
END GET_NEXT_OFFER;
...
BEGIN
  PROCS_LICENSE_V24.UPDATE_LICENSE_STATUS (
    in_license_id,
    in_license_status,
    in_updated_by
  );
...
BEGIN
  PROCS_SUBSCRIPTION_V24.HAS_FUTURE_LICENSE (
    in_license_id,
    out_result
  );
END HAS_FUTURE_LICENSE;
...
BEGIN
  PROCS_LICENSE_V24.GET_GROUP_ID_BY_LICENSE_ID (
    in_license_id,
    out_group_id
  );
END GET_GROUP_ID_BY_LICENSE_ID;
...
BEGIN
  PROCS_OFFER_CHAIN_V24.GET_OFFER_PRODUCTS (
    in_offer_id,
    out_result_set
  );
END GET_OFFER_PRODUCTS;
...
BEGIN
  PROCS_TRANSACTION_V24.CREATE_TRANSACTION(
    ...
    in_created_by,
    in_order_id,
    GLOBAL_CONSTANTS_V24.FALSE, -- is_refund should be false in renewal
    in_transaction_type_code,
    out_transaction_id
  );
END CREATE_TRANSACTION;
...
BEGIN
  PROCS_TRANSACTION_V24.RESERVE_TRANSACTION_ID (
    out_transaction_id
  );
END RESERVE_TRANSACTION_ID;
...
BEGIN
  PROCS_OFFER_CHAIN_V24.P_GET_NEXT_OFFER_INDEX(
    in_offer_chain_id,
    in_offer_chain_current_index,
...
BEGIN
  PROCS_OFFER_CHAIN_V24.GET_NEXT_OFFER_INDEX_BY_LCNS(
    in_license_id,
    in_offer_chain_current_index,
    out_next_offer_index
  );
...
BEGIN
  PROCS_SUBSCRIPTION_V24.CLOSE_SUBSCRIPTION(
    in_subscription_id,
    in_updated_by
  );
END CLOSE_SUBSCRIPTION;
...
BEGIN
  PROCS_LICENSE_V24.GET_NEED_ENTITLEMENTS_LICENSES(
    out_result_set
  );
END GET_NEED_ENTITLEMENTS_LICENSES;
...
BEGIN
  PROCS_LICENSE_V24.UPDATE_NEED_ENTITLEMENTS_FLAG(
    in_license_id,
    in_needs_entitlements,
    in_updated_by
  );
...
BEGIN
  PROCS_OFFER_CHAIN_V24.GET_PROD_OFFERINGS_BY_OFFER_ID(in_offer_id,out_result_set);
END GET_PROD_OFFERINGS_BY_OFFER_ID;
...
BEGIN
  PROCS_INVOICE_V24.GET_INVOICE_LINE_ITEMS(
    in_invoice_id,
    out_result_set
  );
END GET_INVOICE_LINE_ITEMS;
...
BEGIN
  PROCS_PRODUCT_V24.GET_PRD_OFFERING_BY_LINE_IT_ID(
    in_line_item_id,
    out_result_set
  );
END GET_PRD_OFFERING_BY_LINE_IT_ID;
...
BEGIN
  PROCS_LINE_ITEMS_V24.GET_LINE_ITEM_DISCOUNTS(
    in_line_item_id,
    out_result_set
  );
END GET_LINE_ITEM_DISCOUNTS;
...
BEGIN
  PROCS_LINE_ITEMS_V24.UPDATE_LINE_ITEM_AMOUNT(
    in_line_item_id,
    in_amount,
    in_discount_amount,
    in_taxes_amount
...
BEGIN
  PROCS_OFFER_CHAIN_V24.GET_ALL_META_DATA (
    in_offer_chain_id,
    out_result_set
  );
...
BEGIN
  PROCS_SUBSCRIPTION_V24.GET_SUBSCRIPTIONS_META_DATA(
...
BEGIN
  PROCS_FIN_INSTRUMENTS_V24.GET_UNREDEEMED_GCS(
    out_result_set => out_result_set,
...
AS
BEGIN
  PROCS_SUBSCRIPTION_V24.GET_ACT_SUBS_W_CPT_CHARGEBACKS(
    out_result_set => out_result_set
  );
END GET_ACT_SUBS_W_CPT_CHARGEBACKS;
...
AS
BEGIN
  PROCS_SUBSCRIPTION_V24.GET_ACT_SUBS_W_PP_CHARGEBACKS(
    out_result_set => out_result_set
  );
END GET_ACT_SUBS_W_PP_CHARGEBACKS;
...
AS
BEGIN
  PROCS_SUBSCRIPTION_V24.GET_GRACE_PERIOD_SUB_REGIS(
...
AS
BEGIN
  PROCS_SUBSCRIPTION_V24.GET_ACT_SUBS_W_AMEX_CB(
    out_result_set => out_result_set
  );
END GET_ACT_SUBS_W_AMEX_CB;
...
BEGIN
  PROCS_LICENSE_V24.GET_GRACE_LICE_FOR_FINAL_TRANS(
...
BEGIN
  PROCS_FIN_INSTRUMENTS_V24.GET_EXPIRING_PAYPAL(
    in_expire_window_days   => in_expire_window_days,
    in_creation_limit_days  => in_creation_limit_days,
    in_retry_throttle_name  => in_retry_throttle_name,
    out_result_set          => out_result_set
  );
END GET_EXPIRING_PAYPAL;
...
prompt Compiling body for public_procs_client.sql
CREATE OR REPLACE PACKAGE BODY "PUBLIC_PROCS_CLIENT_V24" AS
...
BEGIN
  PROCS_NOTIFICATION_V24.GET_NOTIFICATION_TYPE_BY_NAME (
...
BEGIN
  PROCS_NOTIFICATION_V24.ADD_NOTIFICATION (
    in_sender_account_id,
    in_recipient_group_id,
    in_notification_type_id,
    in_date_to_notify,
...
prompt Compiling body for procs_ar_bal_applies.sql
CREATE OR REPLACE PACKAGE BODY "PROCS_AR_BAL_APPLIES_V24" AS
...
SPROC_NAME      CONSTANT VARCHAR2(25) := 'APPLY_BALANCE';
BEGIN

    INSERT INTO  AR_BALANCE_APPLIES(
        INVOICE_ID,
        CHARGE_ID,
        AMOUNT,
        CREATED_BY,
...
grant execute on CORE_OWNER.PROCS_ACCOUNT_V24 to core_app;
grant execute on CORE_OWNER.PROCS_ADX_V24 to core_app;
grant execute on CORE_OWNER.PROCS_CHARGE_V24 to core_app;
grant execute on CORE_OWNER.PROCS_COMMON_V24 to core_app;
grant execute on CORE_OWNER.PROCS_FIN_INSTRUMENTS_V24 to core_app;
grant execute on CORE_OWNER.PROCS_INVOICE_V24 to core_app;
grant execute on CORE_OWNER.PROCS_LICENSE_V24 to core_app;
grant execute on CORE_OWNER.PROCS_LINE_ITEMS_V24 to core_app;
grant execute on CORE_OWNER.PROCS_OFFER_CHAIN_V24 to core_app;
grant execute on CORE_OWNER.PROCS_PRODUCT_V24 to core_app;
grant execute on CORE_OWNER.PROCS_SUBSCRIPTION_V24 to core_app;
grant execute on CORE_OWNER.PROCS_SYSTEM_V24 to core_app;
grant execute on CORE_OWNER.PROCS_TEST_V24 to core_app;
grant execute on CORE_OWNER.PROCS_TRANSACTION_V24 to core_app;
grant execute on CORE_OWNER.PROCS_TAXES_V24 to core_app;
grant execute on CORE_OWNER.PROCS_LOCKING_V24 to core_app;
grant execute on CORE_OWNER.PROCS_ADJUSTMENTS_V24 to core_app;
grant execute on CORE_OWNER.PUBLIC_PROCS_BILLING_V24 to core_app;
grant execute on CORE_OWNER.PROCS_ADDRESS_V24 to core_app;
grant execute on CORE_OWNER.PROCS_GROUP_ACCOUNT_V24 to core_app;

grant execute on CORE_OWNER.PUBLIC_PROCS_BILLING_V24 to core_billing_app;
grant execute on CORE_OWNER.PUBLIC_PROCS_RENEWAL_V24 to core_licensing_app;
grant execute on CORE_OWNER.PROCS_GROUP_ACCOUNT_V24 to core_licensing_app;
grant execute on CORE_OWNER.PUBLIC_PROCS_NOTIFICATION_V24 to ops_notif_app;


grant execute on CORE_OWNER.PROCS_TAXES_V24 to core_tax_app;
grant execute on CORE_OWNER.PROCS_PRODUCT_V24 to core_tax_app;
grant execute on CORE_OWNER.PROCS_ACCOUNT_V24 to core_tax_app;

grant execute on CORE_OWNER.PROCS_POLLING_SYNC to core_poller_app;

grant execute on CORE_OWNER.PROCS_RECONCILIATION_CRU_V24 to etl_app;
grant execute on CORE_OWNER.PROCS_SUBSCRIPTION_V24 to etl_app;
grant execute on CORE_OWNER.PROCS_TRANSACTION_V24 to etl_app;
grant execute on CORE_OWNER.PROCS_REPORTING to etl_app;
grant execute on CORE_OWNER.PROCS_LOCKING_V24 to etl_app;
grant execute on CORE_OWNER.PROCS_ACCOUNT_V24 to etl_app;
grant execute on CORE_OWNER.PROCS_INVOICE_V24 to etl_app;

grant execute on CORE_OWNER.PROCS_ITUNES_RECEIPT_V24 to core_app;
grant execute on CORE_OWNER.PROCS_ITUNES_RECEIPT_V24 to core_subup_app;
grant execute on CORE_OWNER.PROCS_AMAZON_V24 to core_licensing_app;
grant execute on CORE_OWNER.PROCS_AMAZON_V24 to core_app;
grant execute on CORE_OWNER.PROCS_AMAZON_V24 to core_subup_app;
grant execute on CORE_OWNER.PROCS_LICENSE_V24 to core_subup_app;
grant execute on CORE_OWNER.PROCS_LOCKING_V24 to core_subup_app;

grant execute on CORE_OWNER.PROCS_SUBSCRIPTION_V24 to core_subup_app;
grant execute on CORE_OWNER.PROCS_GROUP_ACCOUNT_V24 to core_app;
grant execute on CORE_OWNER.PROCS_GROUP_ACCOUNT_CRU_V24 to core_app;
grant execute on CORE_OWNER.PROCS_ENTITLEMENT_V24 to core_app;

grant execute on CORE_OWNER.PROCS_CUPY to cupy_app;
grant execute on CORE_OWNER.PROCS_LOCKING_V24 to cupy_app;

grant execute on CORE_OWNER.NOTIFICATION_STATUSES_V24 to ops_notif_app;
grant execute on CORE_OWNER.PROCS_NOTIFICATION_V24 to ops_notif_app;
grant execute on CORE_OWNER.PROCS_SYSTEM_V24 to ops_notif_app;

grant execute on CORE_OWNER.PUBLIC_PROCS_CLIENT_V24 to core_licensing_app;
grant execute on CORE_OWNER.PUBLIC_PROCS_CLIENT_V24 to core_billing_app;
grant execute on CORE_OWNER.PUBLIC_PROCS_CLIENT_V24 to core_app;
grant execute on CORE_OWNER.PUBLIC_PROCS_CLIENT_V24 to cupy_app;

grant execute on CORE_OWNER.PROCS_PROCESS_RETRY_V24 to cupy_app;
grant execute on CORE_OWNER.PROCS_PROCESS_RETRY_V24 to core_owner;
grant execute on CORE_OWNER.PROCS_PROCESS_RETRY_V24 to core_app;
grant execute on CORE_OWNER.PROCS_PROCESS_RETRY_V24 to core_billing_app;
grant execute on CORE_OWNER.PROCS_PROCESS_RETRY_V24 to core_licensing_app;
grant execute on CORE_OWNER.PROCS_PROCESS_RETRY_V24 to ops_notif_app;
grant execute on CORE_OWNER.PROCS_PROCESS_RETRY_V24 to etl_app;
grant execute on CORE_OWNER.PROCS_PROCESS_RETRY_V24 to core_subup_app;
grant execute on CORE_OWNER.PROCS_PROCESS_RETRY_V24 to core_poller_app;
grant execute on CORE_OWNER.PROCS_PROCESS_RETRY_V24 to core_subup_app;
...
Grant Execute On Core_Owner.App_Exception_Codes_V24 To Core_Hist_Owner;

Grant Execute On Core_Owner.GLOBAL_ENUMS_V24 To Core_Hist_Owner;

Grant Execute On Core_Owner.Global_Statuses_V24 To Core_Hist_Owner;

GRANT EXECUTE ON CORE_OWNER.PROCS_AR_BAL_APPLIES_V24 TO core_app;
GRANT EXECUTE ON CORE_OWNER.PROCS_AR_BAL_APPLIES_V24 TO core_billing_app;
grant execute on core_owner.procs_entitlement_V24 to lire_app;
grant execute on core_owner.procs_adx_V24 to lire_app;

COMMENT ON TABLE SYS_VERSION IS 'Bulk SQL generated by ruben.cepeda@nytimes.com on Wed Aug 14 16:09:29 EDT 2013';
