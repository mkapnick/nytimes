...
CREATE OR REPLACE PACKAGE "APP_EXCEPTION_CODES_V18" 
AS
...
CREATE OR REPLACE PACKAGE "GLOBAL_CONSTANTS_V18" AS 
...
CREATE OR REPLACE PACKAGE "GLOBAL_ENUMS_V18" AS 
...
CREATE OR REPLACE PACKAGE "GLOBAL_STATUSES_V18" AS
...
CREATE OR REPLACE PACKAGE "NOTIFICATION_STATUSES_V18" AS
...
CREATE OR REPLACE PACKAGE "PROCS_COMMON_V18" AS 
...
END PROCS_COMMON_V18;
.
/

CREATE OR REPLACE PACKAGE BODY "PROCS_COMMON_V18" AS
...
BEGIN
  PROCS_COMMON_V18.ISO8601DURATION_TO_INTERVALS(in_duration, var_years, var_months, var_days);
...
BEGIN
  IF in_error_stacktrace IS NOT NULL THEN
    RETURN in_sproc_name || ': ' || in_error_message || ': ' || GLOBAL_CONSTANTS_V18.EXCEPTION_DELIMETER || in_error_stacktrace;
  END IF;
  RETURN
...
grant execute on CORE_OWNER.APP_EXCEPTION_CODES_V18 to ops_hist_owner;

grant execute on CORE_OWNER.GLOBAL_ENUMS_V18 to core_hist_owner;
grant execute on CORE_OWNER.GLOBAL_ENUMS_V18 to ops_owner;
grant execute on CORE_OWNER.GLOBAL_ENUMS_V18 to ops_hist_owner;
...
grant execute on CORE_OWNER.GLOBAL_STATUSES_V18 to ops_hist_owner;
