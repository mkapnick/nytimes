...
CREATE OR REPLACE PACKAGE "APP_EXCEPTION_CODES_V15" 
AS
...
CREATE OR REPLACE PACKAGE "GLOBAL_CONSTANTS_V15" AS 
...
CREATE OR REPLACE PACKAGE "GLOBAL_ENUMS_V15" AS 
...
CREATE OR REPLACE PACKAGE "GLOBAL_STATUSES_V15" AS
...
/*
Next statuses were deleted by Sergey.
INVOICE_PENDING_TRANSACTION CONSTANT NUMBER := 2; -- IN SPECS INVOICE_PENDING_TRANSACTION
INVOICE_LEGACY              CONSTANT NUMBER := 4;
INVOICE_FAILED              CONSTANT NUMBER := 5; -- FAILED INVOICE
INVOICE_CANCELED            CONSTANT NUMBER := 6; -- CANCELED INVOICE
*/
...
SUBSCR_CANC_REASON_INACTIVE CONSTANT NUMBER := 1;
...
BEGIN
  PROCS_COMMON_V15.ISO8601DURATION_TO_INTERVALS(in_duration, var_years, var_months, var_days);
...
BEGIN
  IF in_error_stacktrace IS NOT NULL THEN
    RETURN in_sproc_name || ': ' || in_error_message || ': ' || GLOBAL_CONSTANTS_V15.EXCEPTION_DELIMETER || in_error_stacktrace;
  END IF;
  RETURN
...
grant execute on CORE_OWNER.APP_EXCEPTION_CODES_V15 to core_hist_owner;
grant execute on CORE_OWNER.APP_EXCEPTION_CODES_V15 to ops_owner;
grant execute on CORE_OWNER.APP_EXCEPTION_CODES_V15 to ops_hist_owner;

grant execute on CORE_OWNER.GLOBAL_ENUMS_V15 to core_hist_owner;
grant execute on CORE_OWNER.GLOBAL_ENUMS_V15 to ops_owner;
grant execute on CORE_OWNER.GLOBAL_ENUMS_V15 to ops_hist_owner;

grant execute on CORE_OWNER.GLOBAL_STATUSES_V15 to core_hist_owner;
grant execute on CORE_OWNER.GLOBAL_STATUSES_V15 to ops_owner;
grant execute on CORE_OWNER.GLOBAL_STATUSES_V15 to ops_hist_owner;
