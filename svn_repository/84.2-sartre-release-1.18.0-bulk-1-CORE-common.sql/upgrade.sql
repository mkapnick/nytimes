...
CREATE OR REPLACE PACKAGE "APP_EXCEPTION_CODES_V24" 
AS
...
CREATE OR REPLACE PACKAGE "GLOBAL_CONSTANTS_V24" AS 
...
CREATE OR REPLACE PACKAGE "GLOBAL_ENUMS_V24" AS 
...
CREATE OR REPLACE PACKAGE "GLOBAL_STATUSES_V24" AS
...
CHARGE_CAPTURED  CONSTANT NUMBER := 4;
CHARGE_FAILED  CONSTANT NUMBER := 5;
CHARGE_SETTLED  CONSTANT NUMBER := 6;
CHARGE_CHARGEBACK_CLAIM  CONSTANT NUMBER := 7;
CHARGEBACK_SETTLED  CONSTANT NUMBER := 8;
VOID CONSTANT NUMBER := 9;
...
CREATE OR REPLACE PACKAGE "NOTIFICATION_STATUSES_V24" AS
...
CREATE OR REPLACE PACKAGE "PROCS_COMMON_V24" AS 
...
END PROCS_COMMON_V24;
.
/

CREATE OR REPLACE PACKAGE BODY "PROCS_COMMON_V24" AS
...
BEGIN
  PROCS_COMMON_V24.ISO8601DURATION_TO_INTERVALS(in_duration, var_years, var_months, var_days);
...
BEGIN
  IF in_error_stacktrace IS NOT NULL THEN
    RETURN in_sproc_name || ': ' || in_error_message || ': ' || GLOBAL_CONSTANTS_V24.EXCEPTION_DELIMETER || in_error_stacktrace;
  END IF;
  RETURN
